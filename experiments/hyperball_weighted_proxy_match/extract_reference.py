"""Extract dense parameter-count-weighted base-LR/raw-update-RMS targets."""
    ...
    try:
        with source.open('rb') as inp,temp.open('w',encoding='utf-8') as out:
            def flush():
                ...
                target=math.fsum(k*base_lr/v for k,v in zip(numels,rms))/N
                out.write(json.dumps(dict(update_step=step,pre_update_step=step-1,target=target,
        ...
        metadata=dict(metric='weighted_base_lr_over_raw_update_rms',names=names,numels=numels,
            total_controlled_numel=N,rows=rows,source=str(source.resolve()),source_sha256=digest.hexdigest(),
            tensor_metadata_sha256=hashlib.sha256(meta_bytes).hexdigest(),
            definition='sum_i (numel_i/N) * block_base_lr / RMS(raw_update_i); WTE once, gamma excluded; no interpolation')
