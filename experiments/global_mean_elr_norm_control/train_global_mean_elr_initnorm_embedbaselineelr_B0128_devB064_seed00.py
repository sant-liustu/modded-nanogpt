...
# Purpose: single-ELR AdamW consistency test at B=128, T=1024, 20400 steps.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Block matrices use lr_i(t) = target_elr(t) * RMS(W_i(t)); tied wte/head uses 2x target ELR.
# RMSNorm gamma remains on the baseline AdamW WSD schedule. Weight decay stays fixed.
import os
import random
import sys
...
class Hyperparameters:
    ...
    global_mean_elr_reference_json : str = os.environ.get(
        'GLOBAL_MEAN_ELR_REFERENCE_JSON',
        'experiments/global_mean_elr_norm_control/global_mean_elr_reference.jsonl',
    )
    single_elr_log_every : int = 4
    single_elr_detailed_log_every : int = 1000
...
controlled_optimizer_groups = [
    dict(
        params=[p],
        lr=1.0,  # overwritten by the global-mean-ELR controller before every optimizer step
        weight_decay=0.0,
        global_mean_elr_name=name,
    )
...
def load_global_mean_elr_targets(path):
    targets = {}
    with open(path, 'r', encoding='utf-8-sig') as f:
        for line_number, line in enumerate(f, start=1):
            ...
            block_mean_elr = float(record['target_block_mean_elr'])
            embedding_elr = float(record['target_embedding_elr'])
            if update_step in targets:
                raise ValueError(f'duplicate global-mean-ELR target for update_step={update_step}')
            if not all(math.isfinite(value) and value > 0.0 for value in (block_mean_elr, embedding_elr)):
                raise ValueError(f'invalid global-mean-ELR target at line {line_number}')
            targets[update_step] = (block_mean_elr, embedding_elr)
    ...
    if missing_steps or extra_steps:
        raise ValueError(
            'global-mean-ELR reference must contain exactly one target for every update; '
            f'missing={missing_steps[:10]}, extra={extra_steps[:10]}'
        )
    return targets

global_mean_elr_targets = load_global_mean_elr_targets(args.global_mean_elr_reference_json)
initial_controlled_rms = [
    (p.detach().float().square().mean().sqrt()).item()
    for p in controlled_2d_parameters
]

def should_log_single_elr(update_step):
    return (
        master_process
        and args.single_elr_log_every > 0
        and (
            update_step % args.single_elr_log_every == 0
            or update_step == 1
...
@torch.no_grad()
def maybe_apply_global_mean_elr_controller(update_step):
    target_block_mean_elr, target_embedding_elr = global_mean_elr_targets[update_step]
    ...
    block_rms = torch.cat((rms_values[:embedding_controlled_index], rms_values[embedding_controlled_index + 1:]))
    global_block_lr = target_block_mean_elr / torch.mean(1.0 / block_rms).item()
    learning_rates = [global_block_lr] * len(controlled_2d_parameters)
    learning_rates[embedding_controlled_index] = rms_values[embedding_controlled_index].item() * target_embedding_elr
    ...
    if should_log_single_elr(update_step):
        block_actual_elrs = [
            actual_elr
        ...
        record = dict(
            ...
            target_block_mean_elr=target_block_mean_elr,
            target_embedding_elr=target_embedding_elr,
            global_block_lr=global_block_lr,
            ...
            block_mean_elr_relative_error=abs(sum(1.0 / value for index, value in enumerate(rms_values) if index != embedding_controlled_index) * global_block_lr / 72.0 - target_block_mean_elr) / target_block_mean_elr,
            weight_decay=0.0,
            elr_definition='learning_rate / parameter_rms',
        )
        if (
            args.single_elr_detailed_log_every > 0
            and (
                update_step % args.single_elr_detailed_log_every == 0
                or update_step == 1
                or update_step == args.num_iterations
            )
        ):
        ...
        with open(os.path.join(logdir, 'global_mean_elr_history.jsonl'), 'a') as f:
            f.write(json.dumps(record) + '\n')

@torch.no_grad()
def project_controlled_2d_to_initial_rms():
    for parameter, target_rms in zip(controlled_2d_parameters, initial_controlled_rms):
        current_rms = parameter.detach().float().square().mean().sqrt()
        parameter.mul_((target_rms / current_rms).to(dtype=parameter.dtype))
...
def maybe_log_adamw_update_norms(update_step, update_state):
    ...
    with torch.no_grad(), open(history_path, 'a') as f:
        for raw_name, tensor in raw_model.named_parameters():
...
def maybe_log_tensor_norms(step):
    ...
    named_parameters = [
        (canonical_parameter_name(name), tensor)
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    maybe_apply_global_mean_elr_controller(update_step)
    ...
    project_controlled_2d_to_initial_rms()
    # null the gradients
