...
class Hyperparameters:
    ...
    single_elr_reference_json : str = os.environ.get(
        'SINGLE_ELR_REFERENCE_JSON',
...
single_elr_multipliers = torch.tensor(
    [2.0 if name == 'transformer.wte.weight' else 1.0 for name in controlled_2d_names],
    device=device,
    dtype=torch.float32,
)
...
controlled_optimizer_groups = [
    dict(
        params=[p],
        lr=1.0,  # overwritten by the single-ELR controller before every optimizer step
        weight_decay=args.weight_decay,
        single_elr_name=name,
    )
...
def load_single_elr_targets(path):
    targets = {}
    with open(path, 'r', encoding='utf-8-sig') as f:
        for line_number, line in enumerate(f, start=1):
            ...
            target_elr = float(record['target_elr'])
            if update_step in targets:
                raise ValueError(f'duplicate single-ELR target for update_step={update_step}')
            if not math.isfinite(target_elr) or target_elr <= 0.0:
                raise ValueError(
                    f'invalid target_elr={target_elr} at line {line_number}; expected finite and > 0'
                )
            targets[update_step] = target_elr
    ...
    if missing_steps or extra_steps:
        raise ValueError(
            'single-ELR reference must contain exactly one target for every update; '
            f'missing={missing_steps[:10]}, extra={extra_steps[:10]}'
        )
    return targets

single_elr_targets = load_single_elr_targets(args.single_elr_reference_json)
...
@torch.no_grad()
def maybe_apply_single_elr_controller(update_step):
    target_elr = single_elr_targets[update_step]
    ...
    target_elr_values = single_elr_multipliers * target_elr
    learning_rates = (rms_values * target_elr_values).tolist()  # one host sync for all 73 tensors
    rms_values = rms_values.tolist()
    target_elr_values = target_elr_values.tolist()
    ...
    if should_log_single_elr(update_step):
        ...
        record = dict(
            ...
            target_block_elr=target_elr,
            target_embedding_elr=2.0 * target_elr,
        ...
        with open(os.path.join(logdir, 'single_elr_history.jsonl'), 'a') as f:
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    maybe_apply_single_elr_controller(update_step)
