...
# Purpose: KDA--full-attention 3:1 hybrid AdamW experiment at B=128, T=1024, 20400 steps.
# Layers 1--3, 5--7, and 9--11 use FLA KimiDeltaAttention; layers 4, 8, and 12
# retain the original QK-norm RoPE causal full attention. Install FLA on the CUDA
# training machine before running: pip install "flash-linear-attention[cuda]"
...
class KDAAttention(nn.Module):
    ...
    A full [B, T, C] training block is passed to FLA at once. In training mode,
    mode="chunk" selects FLA's chunk-wise parallel kernel; use_cache=False only
    disables persistence of recurrent state across separate forward calls.
    """
    ...
    def __init__(self, config, layer_idx):
        ...
        # The remaining KDA-specific parameters retain FLA's native initialization.
...
class Block(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        super().__init__()
        # One-indexed layers 4, 8, and 12 remain original full attention.
        self.attn = (
