...
# Purpose: independent eager/no-compile control for the 586.3M-total /
# 140.4M-active Qwen3-MoE AdamW run with the same GPT-2 tokenizer/data and
# tied embedding.
...
# Experts use the original readable one-hot/where/index_add_ dispatcher. This
# control intentionally uses neither grouped GEMM nor torch compilation.
import os
import random
import sys
...
class Hyperparameters:
    ...
    # This control deliberately keeps the full model in ordinary eager mode.
...
if master_process:
    ...
    print("Qwen3-MoE expert backend: eager Python dispatcher (compilation disabled)")
model = model.cuda()
