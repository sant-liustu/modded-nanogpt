...
def verify_stage(path, cfg):
    ...
    rows=[json.loads(line) for line in (path/'global_elr_history.jsonl').read_text(encoding='utf-8').splitlines()]
    ...
    for r in rows:
        ...
        reconstructed_f=math.sqrt(sum(k*v*v for k,v in zip(r['numels'],r['rms'])))
        assert len(r['numels'])==len(r['rms'])==len(r['names'])
        assert math.isclose(reconstructed_f,r['global_fro_norm'],rel_tol=1e-6)
        assert math.isclose(r['block_lr']/r['global_fro_norm'],r['global_elr'],rel_tol=1e-12)
...
def main():
    from extract_global_f_reference import extract
    if hasattr(sys.stdout,'reconfigure'):sys.stdout.reconfigure(encoding='utf-8',errors='replace')
    def interrupted(signum,frame):raise KeyboardInterrupt(signum)
    signal.signal(signal.SIGTERM,interrupted)
    p=argparse.ArgumentParser(description='Extract new global-F references from existing baselines; run two fresh-start matching jobs concurrently.')
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            futures=[pool.submit(extract,root/f'baseline_{tag}',out/f'global_f_reference_{tag}.jsonl') for tag in tags]
            configs=[f.result() for f in futures]
        event(out,events,stage='references',event='both_global_f_references_ready')
        jobs=[]
        for i,(tag,cfg) in enumerate(zip(tags,configs)):
            cfg=dict(cfg);stage=out/f'global_f_matching_{tag}';stage.mkdir()
            ...
            cfg.update(role='matching',resume_dir='',reference_file=str(out/f'global_f_reference_{tag}.jsonl'),
            ...
            worker=str(HERE/f'train_global_f_{tag}_fresh.py')
        ...
        run_pair(jobs)
        atomic_json(out/'pipeline_complete.json',dict(completed=True,metric='global_elr',fresh_start=True,stages=[j[1].name for j in jobs]))
        print('GLOBAL F MATCHING COMPLETE:',out)
