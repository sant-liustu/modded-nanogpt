# Fresh-start AdamW global-F ELR matching; runtime configuration comes from --config.
# Warmup follows the source baseline; afterwards switch WD and match block_lr/global_F.
# Global F includes block matrices and tied WTE once; WTE LR is 2x block LR.
import os
import argparse
...
assert not args.resume_dir, 'Proxy fresh-start runners never load checkpoints'
...
if master_process:
    ...
    with open(logfile, "w") as f:
        ...
        runtime_record = dict(
            ...
            control_metric='global_elr', actual_wte_lr_multiplier=2.0,
...
if args.role == 'matching':
    with open(args.reference_file, encoding='utf-8') as f:
        for line in f:
            ...
            assert math.isfinite(row['global_elr']) and row['global_elr'] > 0
            reference[step_id] = row
    assert set(reference) == set(range(1, args.num_iterations + 1))

@torch.no_grad()
def record_or_match_global_elr(update_step):
    ...
    for _, p in controlled_named:
        total_sq.add_(p.detach().float().square().sum())
    global_f = total_sq.sqrt().item()
    if not math.isfinite(global_f) or global_f <= 0: raise RuntimeError('Invalid global F')
    target = None
    if update_step > args.warmup_iters:
        target = reference[update_step]['global_elr']
        lr = target * global_f
    ...
    error = None if target is None else abs(global_elr/target-1.0)
    ...
    if master_process:
        record = dict(update_step=update_step, pre_update_step=update_step-1, names=controlled_names,
                      ...
                      global_elr=global_elr, global_fro_norm=global_f, numels=controlled_numels, target_global_elr=target, control_metric='global_elr', relative_error=error, weight_decay=block['weight_decay'])
        with open(Path(args.output_dir) / 'global_elr_history.jsonl', 'a', encoding='utf-8') as f:
            f.write(json.dumps(record)+'\n')
...
for step in range(start_step, args.num_iterations + 1):
    ...
    update_step = step + 1
    record_or_match_global_elr(update_step)
