...
args = Hyperparameters(**supplied)
assert args.role in ('baseline', 'matching')
assert args.output_dir and 0 < args.warmup_iters < args.num_iterations
assert args.compile_model == 1, 'This pipeline requires torch.compile'
...
block_weight_parameters = [
    ...
    if canonical_name(name).startswith('transformer.h.')
...
# Pipeline-only branch checkpoint and dense actual-mean RMS-ELR telemetry.
...
def loader_state(loader):
    return dict(files=loader.files, current_shard=loader.current_shard, current_position=loader.current_position)

def restore_loader(loader, state):
    assert loader.files == state['files'], 'Dataset shard list changed'
    loader.current_shard = state['current_shard']
    loader.current_position = state['current_position']
    loader.tokens = _load_data_shard(loader.files[loader.current_shard])

def rank_state():
    return dict(train_loader=loader_state(train_loader), val_loader=loader_state(val_loader),
                x=x.detach().cpu(), y=y.detach().cpu(), python_rng=random.getstate(),
                numpy_rng=np.random.get_state(), torch_rng=torch.get_rng_state(),
                cuda_rng=torch.cuda.get_rng_state(), spectral_rng=spectral_norm_generator.get_state())

def shared_state(step):
    return dict(step=step, world_size=ddp_world_size, config=supplied,
                model={canonical_name(k): v for k, v in raw_model.state_dict().items()},
                optimizers=[o.state_dict() for o in optimizers], schedulers=[s.state_dict() for s in schedulers])

def save_branch_checkpoint(step):
    path = Path(args.output_dir) / 'warmup_checkpoint'
    path.mkdir(parents=True, exist_ok=True)
    local = rank_state()
    tmp = path / f'rank{ddp_rank}.pt.tmp'
    torch.save(local, tmp); os.replace(tmp, path / f'rank{ddp_rank}.pt')
    if master_process:
        common = shared_state(step)
        torch.save(common, path / 'shared.pt.tmp'); os.replace(path / 'shared.pt.tmp', path / 'shared.pt')
    if use_ddp: dist.barrier()
    if master_process:
        atomic_json(path / 'ready.json', dict(step=step, world_size=ddp_world_size, shared_digest=state_digest(common)))
    if use_ddp: dist.barrier()

controlled_named = [(canonical_name(n), p) for n, p in raw_model.named_parameters()
                   if p.ndim == 2 and (canonical_name(n).startswith('transformer.h.') or canonical_name(n) == 'transformer.wte.weight')]
controlled_named.sort(key=lambda item: (item[0] == 'transformer.wte.weight', item[0]))
controlled_names = [n for n, _ in controlled_named]
assert len(controlled_names) == 6 * args.n_layer + 1 and len(set(controlled_names)) == len(controlled_names)
assert controlled_names[-1] == 'transformer.wte.weight'
assert raw_model.transformer.wte.weight is raw_model.lm_head.weight
assert {canonical_name(n) for n, p in model.named_parameters() if p.ndim == 2} == set(controlled_names)
start_step = 0
reference = {}
if args.role == 'matching':
    cp = Path(args.resume_dir)
    ready = json.loads((cp / 'ready.json').read_text(encoding='utf-8'))
    assert ready['step'] == args.warmup_iters and ready['world_size'] == ddp_world_size
    common = torch.load(cp / 'shared.pt', map_location='cpu', weights_only=False)
    assert state_digest(common) == ready['shared_digest']
    for key, val in supplied.items():
        if key not in {'role', 'output_dir', 'resume_dir', 'reference_file', 'post_weight_decay'}:
            assert common['config'][key] == val, f'Baseline configuration mismatch: {key}'
    restored = {canonical_name(k): v for k, v in common['model'].items()}
    raw_model.load_state_dict(restored, strict=True)
    for opt, state in zip(optimizers, common['optimizers']): opt.load_state_dict(state)
    for sched, state in zip(schedulers, common['schedulers']): sched.load_state_dict(state)
    local = torch.load(cp / f'rank{ddp_rank}.pt', map_location='cpu', weights_only=False)
    restore_loader(train_loader, local['train_loader']); restore_loader(val_loader, local['val_loader'])
    x, y = local['x'].to(device), local['y'].to(device)
    random.setstate(local['python_rng']); np.random.set_state(local['numpy_rng'])
    torch.set_rng_state(local['torch_rng']); torch.cuda.set_rng_state(local['cuda_rng'])
    spectral_norm_generator.set_state(local['spectral_rng'])
    assert state_digest(rank_state()) == state_digest(local), 'RNG/loader/prefetch restoration mismatch'
    assert state_digest(raw_model.state_dict()) == state_digest(restored), 'Model restoration mismatch'
    ...
    with open(args.reference_file, encoding='utf-8') as f:
        for line in f:
            row = json.loads(line)
            names = [canonical_name(n) for n in row['names']]
            assert names == controlled_names, 'Reference tensor identities/order mismatch'
            step_id = row['update_step']
            assert step_id not in reference
            assert row['pre_update_step'] == step_id - 1
            assert math.isfinite(row['actual_mean_elr']) and row['actual_mean_elr'] > 0
            reference[step_id] = row
    assert set(reference) == set(range(1, args.num_iterations + 1))

@torch.no_grad()
def record_or_match_mean_elr(update_step):
    # Deliberately compute outside torch.compile on canonical base-model parameters.
    rms = torch.stack([p.detach().float().square().mean().sqrt() for _, p in controlled_named])
    if not bool(torch.isfinite(rms).all() and (rms > 0).all()):
        raise RuntimeError('Non-positive/non-finite matrix RMS')
    values = rms.double().cpu().tolist()
    inverse_sum = sum(1.0/v for v in values[:-1]) + 2.0/values[-1]
    target = None
    if args.role == 'matching':
        assert update_step > args.warmup_iters
        target = reference[update_step]['actual_mean_elr']
        lr = len(values) * target / inverse_sum
        for group in optimizer1.param_groups:
            group['lr'] = 2.0 * lr; group['weight_decay'] = args.post_weight_decay
        for group in optimizer2.param_groups:
            if group.get('lrnorm_match_group', False):
                group['lr'] = lr; group['weight_decay'] = args.post_weight_decay
    block = next(g for g in optimizer2.param_groups if g.get('lrnorm_match_group', False))
    lr = block['lr']; embed_lr = optimizer1.param_groups[0]['lr']
    assert math.isclose(embed_lr, 2.0*lr, rel_tol=1e-12)
    actual = (sum(lr/v for v in values[:-1]) + embed_lr/values[-1]) / len(values)
    error = None if target is None else abs(actual/target-1.0)
    if error is not None and error > 1e-10: raise RuntimeError(f'ELR alignment failure: {error}')
    if master_process:
        record = dict(update_step=update_step, pre_update_step=update_step-1, names=controlled_names,
                      rms=values, block_lr=lr, embed_lr=embed_lr, actual_mean_elr=actual,
                      target_mean_elr=target, relative_error=error, weight_decay=block['weight_decay'])
        with open(Path(args.output_dir) / 'dense_mean_elr.jsonl', 'a', encoding='utf-8') as f:
            f.write(json.dumps(record)+'\n')
...
for step in range(start_step, args.num_iterations + 1):
    if step == args.warmup_iters and args.role == 'baseline':
        save_branch_checkpoint(step)
    ...
    update_step = step + 1
    record_or_match_mean_elr(update_step)
...
if use_ddp:
    dist.barrier()
if master_process:
    atomic_json(Path(args.output_dir) / 'complete.json', dict(role=args.role, completed_step=args.num_iterations, start_step=start_step, config=supplied))
# clean up nice
if use_ddp:
