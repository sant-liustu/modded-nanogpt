...
# Purpose: drop block weight decay after warmup and match cosine wd0p1 block LR / total block norm.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, warmup+cosine, block_weight_decay=0.1 -> 0.0 after update 1000, seed=0
import os
import random
import sys
...
class Hyperparameters:
    ...
    seed : int = 0
    lrnorm_reference_json : str = 'experiments/lrnorm_match/reference_rmsnorm_gamma_adamw_wd0p1_cosine_rep01_block_plus_embedding_total_lr_over_norm.jsonl'
    lrnorm_match_start_update : int = 1001 # first update after warmup: drop block WD and match LR / block+embedding total norm
