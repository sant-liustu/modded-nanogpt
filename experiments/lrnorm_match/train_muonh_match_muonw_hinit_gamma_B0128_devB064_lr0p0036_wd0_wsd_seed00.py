...
# Variant: dynamically match MuonH LR/raw-update to the paired MuonW LR/norm reference with the same MuonH initialization.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, warmup+stable+warmdown, block_weight_decay=0.0, seed=0; controller target=MuonW wd0 WSD.
import os
import random
import sys
...
class Hyperparameters:
    ...
    # Generate this from the paired MuonW+MuonH-initialization baseline after it finishes.
    muonw_lrnorm_reference_json : str = 'experiments/lrnorm_match/reference_rmsnorm_gamma_qknorm_muonw_muonhinit_wd0_wsd_per_tensor_lr_over_norm.jsonl.gz'
