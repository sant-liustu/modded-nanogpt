# Copied from experiments/small_batch_adamw_baseline generated AdamW wd0p1 baseline.
# Purpose: add block weight decay after warmup and match wd0 LR_t * ||AdamW update_{t-1}|| / total parameter norm.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, block_weight_decay=0.0 -> 0.1 after update 1000, seed=0
import os
import random
import sys
...
class Hyperparameters:
    ...
    weight_decay : float = 0.0 # initial weight decay for block weights and tied wte/lm_head; RMSNorm gamma is excluded
    ...
    seed : int = 0
    lrupdate_reference_json : str = 'experiments/lrnorm_match/reference_rmsnorm_gamma_adamw_wd0_rep01_block_plus_embedding_total_lag1_lr_update_over_norm.jsonl'
    lrupdate_match_start_update : int = 1001 # first update after warmup: add block WD and match LR_t * ||update_{t-1}|| / block+embedding total norm
    lrupdate_match_log_every : int = 1
...
lrupdate_targets = load_lrnorm_targets(args.lrupdate_reference_json)
...
lagged_lrupdate_state = dict(update_step=None, scaled_update_norm=None)

def should_log_lrnorm_match(update_step):
    if not master_process or args.lrupdate_match_log_every <= 0:
        return False
    return (
        update_step % args.lrupdate_match_log_every == 0
        or update_step == args.lrupdate_match_start_update
        or update_step == args.num_iterations
    )

def should_track_lagged_lrupdate(update_step):
    return update_step >= args.lrupdate_match_start_update - 1

def maybe_apply_lrnorm_controller(update_step):
    if update_step < args.lrupdate_match_start_update:
        return
    if update_step not in lrupdate_targets:
        raise RuntimeError(f"missing LR*update/norm target for update_step={update_step}")
    target = lrupdate_targets[update_step]
    current_norm = current_lrnorm_denominator_total_fro_norm()
    expected_lagged_update_step = update_step - 1
    current_lagged_scaled_update_norm = lagged_lrupdate_state['scaled_update_norm']
    if lagged_lrupdate_state['update_step'] != expected_lagged_update_step:
        raise RuntimeError(
            f"missing lagged update norm for update_step={update_step}: "
            f"expected previous update {expected_lagged_update_step}, got {lagged_lrupdate_state['update_step']}"
        )
    if current_lagged_scaled_update_norm <= 0.0:
        raise RuntimeError(f"cannot match lagged LR*update/norm at update_step={update_step}: lagged update norm is 0")
    target_lr_update_over_norm = float(target['target_lagged_lr_update_over_norm'])
    adjusted_lr = target_lr_update_over_norm * current_norm / current_lagged_scaled_update_norm
    ...
    adjusted_embed_lr = adjusted_lr * embed_to_block_lr_ratio
    target_weight_decay = 0.1
    ...
    if should_log_lrnorm_match(update_step):
        record = dict(
            ...
            adjusted_block_lr=adjusted_lr,
            adjusted_embed_lr=adjusted_embed_lr,
            block_lr_scale=adjusted_lr / reference_block_lr if reference_block_lr != 0.0 else None,
            embed_lr_scale=adjusted_embed_lr / reference_embed_lr if reference_embed_lr != 0.0 else None,
            lag_updates=int(target.get('lag_updates', 1)),
            reference_lagged_update_step=target.get('reference_lagged_update_step'),
            current_lagged_update_step=lagged_lrupdate_state['update_step'],
            reference_scaled_update_norm=float(target['reference_lagged_scaled_update_norm']),
            current_scaled_update_norm=current_lagged_scaled_update_norm,
            reference_lagged_scaled_update_norm=float(target['reference_lagged_scaled_update_norm']),
            current_lagged_scaled_update_norm=current_lagged_scaled_update_norm,
            reference_current_scaled_update_norm=float(target.get('reference_current_scaled_update_norm', target['reference_lagged_scaled_update_norm'])),
            target_lr_update_over_norm=target_lr_update_over_norm,
            target_lagged_lr_update_over_norm=target_lr_update_over_norm,
            actual_lr_update_over_norm=adjusted_lr * current_lagged_scaled_update_norm / current_norm,
            actual_lagged_lr_update_over_norm=adjusted_lr * current_lagged_scaled_update_norm / current_norm,
            implied_block_lr_over_norm=adjusted_lr / current_norm,
            implied_embed_lr_over_norm=adjusted_embed_lr / current_norm,
            ...
            update_scope=target.get('update_scope', 'optimizer1 tied embedding plus optimizer2 non-gamma block weights'),
        )
        with open(os.path.join(logdir, 'lrupdateparam_match_history.jsonl'), 'a') as f:
    ...
    for optimizer_index, opt in enumerate(optimizers):
        for param_group_index, group in enumerate(opt.param_groups):
            if opt is optimizer1:
                lrupdate_match_multiplier = 2.0
            elif group.get('lrnorm_match_group', False):
                lrupdate_match_multiplier = 1.0
            else:
                lrupdate_match_multiplier = 0.0
            for p in group['params']:
                hparams[id(p)] = dict(
                    ...
                    lrupdate_match_multiplier=lrupdate_match_multiplier,
                )
    return hparams
...
def maybe_capture_adamw_update_state(update_step):
    log_update_norms = should_log_adamw_update_norms(update_step)
    track_lagged_lrupdate = should_track_lagged_lrupdate(update_step)
    if not log_update_norms and not track_lagged_lrupdate:
        return None
    ...
    return dict(
        hparams=hparams,
        snapshots=snapshots,
        log_update_norms=log_update_norms,
        track_lagged_lrupdate=track_lagged_lrupdate,
    )
...
def maybe_log_adamw_update_norms(update_step, update_state):
    ...
    log_update_norms = update_state.get('log_update_norms', False)
    track_lagged_lrupdate = update_state.get('track_lagged_lrupdate', False)
    pending_records = []
    pending_2d_updates = []
    lagged_update_sq = torch.zeros((), device=device, dtype=torch.float32)
    history_file = None
    try:
        if log_update_norms:
        ...
        with torch.no_grad():
            for name, tensor in raw_model.named_parameters():
                ...
                param_hparams = hparams[param_id]
                adamw_update = adamw_update_tensor(tensor, tensor_before, param_hparams, update_step, name)
                lrupdate_match_multiplier = param_hparams.get('lrupdate_match_multiplier', 0.0)
                if track_lagged_lrupdate and lrupdate_match_multiplier != 0.0:
                    lagged_update_sq.add_(adamw_update.float().square().sum() * (lrupdate_match_multiplier * lrupdate_match_multiplier))
                if log_update_norms:
                    if args.spectral_norm_estimate_enabled > 0 and adamw_update.ndim == 2:
                        pending_2d_updates.append((name, adamw_update))
                        pending_records.append((name, tensor, adamw_update, param_hparams))
                    else:
                        record = adamw_update_norm_record(update_step, name, tensor, adamw_update, param_hparams)
        ...
        if history_file is not None:
            history_file.close()
    if track_lagged_lrupdate:
        lagged_lrupdate_state['update_step'] = update_step
        lagged_lrupdate_state['scaled_update_norm'] = torch.sqrt(lagged_update_sq).item()
    if snapshots:
