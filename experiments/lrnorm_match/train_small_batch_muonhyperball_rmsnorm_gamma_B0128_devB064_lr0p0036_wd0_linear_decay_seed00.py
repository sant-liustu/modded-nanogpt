...
# Purpose: linear-decay AdamH + MuonH wd0 experiment with learnable RMSNorm gamma at B=128, T=1024, 20400 steps.
...
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, 1000-step warmup then linear decay, block_weight_decay=0.0, seed=0
import os
import random
import sys
...
# learning rate scheduler (linear warmup followed by linear decay to zero)
def get_lr(it):
    assert it <= args.num_iterations
    if it < args.warmup_iters:
        return (it+1) / args.warmup_iters
    decay_step = it - args.warmup_iters
    return max(0.0, 1.0 - decay_step / args.linear_decay_iters)
