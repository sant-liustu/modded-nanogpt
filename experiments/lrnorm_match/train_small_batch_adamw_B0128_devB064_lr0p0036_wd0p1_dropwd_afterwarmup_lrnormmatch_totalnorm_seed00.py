...
# Purpose: drop block weight decay after warmup and match wd0p1 block LR / total block norm.
...
class Hyperparameters:
    ...
    seed : int = 0
    lrnorm_reference_json : str = 'experiments/lrnorm_match/reference_wd0p1_rep03_block_total_lr_over_norm.jsonl'
    lrnorm_match_start_update : int = 1001 # first update after warmup: drop block WD and match LR / block total norm
...
@torch.no_grad()
def current_block_total_fro_norm():
...
def maybe_apply_lrnorm_controller(update_step):
    ...
    current_norm = current_block_total_fro_norm()
    ...
    if should_log_lrnorm_match(update_step):
        record = dict(
            ...
            reference_block_norm=float(target['reference_block_total_fro_norm']),
            current_block_norm=current_norm,
            reference_block_lr=float(target['reference_block_lr']),
            adjusted_block_lr=adjusted_lr,
            lr_scale=adjusted_lr / float(target['reference_block_lr']) if float(target['reference_block_lr']) != 0.0 else None,
            target_lr_over_norm=target_lr_over_norm,
            actual_lr_over_norm=adjusted_lr / current_norm,
            optimizer2_weight_decay=0.0,
            norm_scope=target.get('norm_scope', 'transformer.h total Frobenius norm'),
        )
