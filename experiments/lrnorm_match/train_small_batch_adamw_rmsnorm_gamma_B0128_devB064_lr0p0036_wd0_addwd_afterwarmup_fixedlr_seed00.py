# Copied from the RMSNorm gamma AdamW wd0 baseline.
# Purpose: add block/tied embedding weight decay after warmup while keeping the baseline LR schedule.
# Token budget matches the B=128, T=1024, 20400-step small-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, weight_decay=0.0 -> 0.1 after update 1000, seed=0
import os
import random
import sys
...
class Hyperparameters:
    ...
    seed : int = 0
    weight_decay_switch_start_update : int = 1001 # first update after warmup: add block and tied wte/lm_head WD
...
optimizer2_groups = [dict(params=block_weight_parameters, weight_decay=args.weight_decay, weight_decay_switch_group=True)]
if rmsnorm_gamma_parameters:
    optimizer2_groups.append(dict(params=rmsnorm_gamma_parameters, weight_decay=0.0, weight_decay_switch_group=False))
optimizer2 = torch.optim.AdamW(optimizer2_groups, lr=0.5*args.embed_learning_rate, betas=(0.9, 0.95),
                               fused=True)
optimizers = [optimizer1, optimizer2]

def maybe_apply_weight_decay_switch(update_step):
    if update_step < args.weight_decay_switch_start_update:
        return
    target_weight_decay = 0.1
    for group in optimizer1.param_groups:
        group['weight_decay'] = target_weight_decay
    for group in optimizer2.param_groups:
        if not group.get('weight_decay_switch_group', False):
            continue
        group['weight_decay'] = target_weight_decay
    if master_process and (update_step == args.weight_decay_switch_start_update or update_step == args.num_iterations):
        record = dict(
            update_step=update_step,
            pre_update_step=update_step - 1,
            optimizer1_lr=float(optimizer1.param_groups[0]['lr']),
            optimizer2_block_lr=float(next(group['lr'] for group in optimizer2.param_groups if group.get('weight_decay_switch_group', False))),
            optimizer1_weight_decay=target_weight_decay,
            optimizer2_weight_decay=target_weight_decay,
            lr_schedule='baseline fixed schedule; no LR/norm matching',
        )
        with open(os.path.join(logdir, 'weight_decay_switch_history.jsonl'), 'a') as f:
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    maybe_apply_weight_decay_switch(update_step)
