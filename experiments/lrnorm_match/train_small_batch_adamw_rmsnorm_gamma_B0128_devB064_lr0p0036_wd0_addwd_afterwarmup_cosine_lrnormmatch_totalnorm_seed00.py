...
# Purpose: add block weight decay after warmup and match cosine wd0 block LR / total block norm.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, warmup+cosine, block_weight_decay=0.0 -> 0.1 after update 1000, seed=0
import os
import random
import sys
...
class Hyperparameters:
    ...
    seed : int = 0
    lrnorm_reference_json : str = 'experiments/lrnorm_match/reference_rmsnorm_gamma_adamw_wd0_cosine_rep01_block_plus_embedding_total_lr_over_norm.jsonl'
    lrnorm_match_start_update : int = 1001 # first update after warmup: add block WD and match LR / block+embedding total norm
...
def maybe_apply_lrnorm_controller(update_step):
    ...
    adjusted_lr = target_lr_over_norm * current_norm
    ...
    if should_log_lrnorm_match(update_step):
        record = dict(
            ...
            actual_block_lr_over_norm=adjusted_lr / current_norm,
...
def optimizer_parameter_hparams():
    param_id_to_name = {}
    for name, p in raw_model.named_parameters():
        param_id_to_name[id(p)] = name.replace('_orig_mod.', '')
    hparams = {}
...
def maybe_capture_adamw_update_state(update_step):
    ...
    with torch.no_grad():
        for name, p in raw_model.named_parameters():
            if p.requires_grad:
                snapshots[name.replace('_orig_mod.', '')] = p.detach().clone()
    ...
    with torch.no_grad(), open(history_path, 'a') as f:
        for name, tensor in raw_model.named_parameters():
            if not tensor.requires_grad:
                continue
            lookup_name = name.replace('_orig_mod.', '')
            if lookup_name not in hparams:
                raise RuntimeError(f"missing optimizer param group mapping for {name}")
            if lookup_name not in snapshots:
                raise RuntimeError(f"missing parameter snapshot for {name}")
            tensor_before = snapshots.pop(lookup_name)
            param_hparams = hparams[lookup_name]
