...
# Purpose: drop block weight decay after warmup and match wd0p1 LR_t * ||AdamW update_{t-1}|| / total parameter norm.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, block_weight_decay=0.1 -> 0.0 after update 1000, seed=0
import os
import random
import sys
...
class Hyperparameters:
    ...
    weight_decay : float = 0.1 # initial weight decay for block weights and tied wte/lm_head; RMSNorm gamma is excluded
    ...
    seed : int = 0
    lrupdate_reference_json : str = 'experiments/lrnorm_match/reference_rmsnorm_gamma_adamw_wd0p1_rep01_block_plus_embedding_total_lag1_lr_update_over_norm.jsonl'
    lrupdate_match_start_update : int = 1001 # first update after warmup: drop block WD and match LR_t * ||update_{t-1}|| / block+embedding total norm
