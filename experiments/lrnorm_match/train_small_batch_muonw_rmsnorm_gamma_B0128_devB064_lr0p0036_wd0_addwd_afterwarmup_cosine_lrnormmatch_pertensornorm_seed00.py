...
# Purpose: cosine MuonW wd0 -> wd0p1 after-warmup LR/norm match with learnable RMSNorm gamma at B=128, T=1024, 20400 steps.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, muon_lr=0.02, warmup+cosine, block_weight_decay=0.0 -> 0.1 after update 1000, seed=0
import os
import random
import sys
...
class Hyperparameters:
    ...
    lrnorm_reference_json : str = 'experiments/lrnorm_match/reference_rmsnorm_gamma_muonw_wd0_cosine_rep01_block_plus_embedding_per_tensor_lr_over_norm.jsonl'
    lrnorm_match_start_update : int = 1001 # first update after warmup: add block WD and match Muon LR / block+embedding total norm
    lrnorm_match_target_weight_decay : float = 0.1
...
def maybe_log_muonw_update_norms(update_step, update_state):
    ...
    with torch.no_grad(), open(history_path, 'a') as f:
        for name, tensor in raw_model.named_parameters():
            ...
            muonw_update = muonw_update_tensor(tensor, tensor_before, param_hparams, update_step, name)
            muonw_update_with_wd = muonw_update_with_wd_tensor(tensor, tensor_before, param_hparams, update_step, name)
            if args.spectral_norm_estimate_enabled > 0 and muonw_update.ndim == 2:
                ...
                pending_records.append((name, tensor, muonw_update, muonw_update_with_wd, param_hparams))
            else:
                record = muonw_update_norm_record(update_step, name, tensor, muonw_update, muonw_update_with_wd, param_hparams)
                f.write(json.dumps(record) + '\n')
                del record
            del tensor_before
