# Copied from the RMSNorm-gamma full-global-F WD 0 -> 0.1 runner.
# Purpose: after warmup, match the WD=0 reference with a 72-block-only
# global-F controller and a separately ELR-matched tied WTE.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024,
# num_iterations=20400, seed=0; block WD changes 0.0 -> 0.1 at update 1001,
# WTE remains outside the block denominator, WD=0, and matches reference ELR.
import os
import random
import sys
...
class Hyperparameters:
    ...
    weight_decay : float = 0.0 # baseline WD; after warmup only controller-marked block matrices switch to 0.1
    ...
    seed : int = 0
    lrnorm_reference_json : str = 'experiments/wte_scope_ablation/reference_rmsnorm_gamma_adamw_wd0_wsd_rep01_blockonly_globalf_wte_elr.jsonl'
    lrnorm_match_start_update : int = 1001 # first update after warmup: add block WD and match block LR / block-only total norm
...
if len(block_weight_parameters) != 72:
    raise RuntimeError(f"expected 72 controlled block matrices, got {len(block_weight_parameters)}")
if any(p is raw_model.transformer.wte.weight for p in block_weight_parameters):
    raise RuntimeError("tied WTE must not enter the block-only global-F denominator")
...
block_global_f_denominator_params = block_weight_parameters
...
@torch.no_grad()
def current_block_global_fro_norm():
    total_sq = torch.zeros((), device=device, dtype=torch.float32)
    for p in block_global_f_denominator_params:
        total_sq.add_(p.detach().float().square().sum())
    return torch.sqrt(total_sq).item()

@torch.no_grad()
def current_tied_wte_rms():
    return raw_model.transformer.wte.weight.detach().float().square().mean().sqrt().item()
...
def maybe_apply_lrnorm_controller(update_step):
    ...
    current_block_fro_norm = current_block_global_fro_norm()
    current_wte_rms = current_tied_wte_rms()
    target_block_lr_over_block_fro_norm = float(target['target_block_lr_over_block_fro_norm'])
    target_wte_elr = float(target['target_wte_elr'])
    adjusted_block_lr = target_block_lr_over_block_fro_norm * current_block_fro_norm
    adjusted_wte_lr = target_wte_elr * current_wte_rms
    reference_block_lr = float(target['reference_block_lr'])
    reference_wte_lr = float(target['reference_wte_lr'])
    reference_wte_weight_decay = float(target['reference_wte_weight_decay'])
    target_block_weight_decay = 0.1

    # WTE remains excluded from F_B. Its private controller is the only
    # difference from the baseline-policy arm.
    for group in optimizer1.param_groups:
        group['lr'] = adjusted_wte_lr
        group['weight_decay'] = reference_wte_weight_decay
    for group in optimizer2.param_groups:
        if not group.get('lrnorm_match_group', False):
            continue
        group['lr'] = adjusted_block_lr
        group['weight_decay'] = target_block_weight_decay

    if should_log_lrnorm_match(update_step):
        record = dict(
            control_arm='block_only_global_f_wte_elr_match',
            update_step=update_step,
            pre_update_step=update_step - 1,
            block_tensor_count=len(block_global_f_denominator_params),
            wte_in_block_global_f_denominator=False,
            reference_block_fro_norm=float(target['reference_block_fro_norm']),
            current_block_fro_norm=current_block_fro_norm,
            reference_block_lr=reference_block_lr,
            adjusted_block_lr=adjusted_block_lr,
            target_block_lr_over_block_fro_norm=target_block_lr_over_block_fro_norm,
            actual_block_lr_over_block_fro_norm=adjusted_block_lr / current_block_fro_norm,
            reference_wte_lr=reference_wte_lr,
            applied_wte_lr=adjusted_wte_lr,
            reference_wte_rms=float(target['reference_wte_rms']),
            current_wte_rms=current_wte_rms,
            target_wte_elr=target_wte_elr,
            actual_wte_elr=adjusted_wte_lr / current_wte_rms,
            wte_lr_is_elr_matched=True,
            optimizer1_weight_decay=reference_wte_weight_decay,
            block_optimizer_weight_decay=target_block_weight_decay,
            norm_scope='72 transformer.h non-gamma matrices only; tied WTE excluded',
        )
