...
def verify_stage(path, cfg):
    ...
    rows=[json.loads(line) for line in (path/'weighted_proxy_elr_history.jsonl').read_text(encoding='utf-8').splitlines()]
    ...
    for r in rows:
        ...
        assert len(r['numels'])==len(r['rms'])==len(r['names'])
        assert sum(r['numels'])==r['total_controlled_numel']
        expected=sum(k*r['block_lr']/v for k,v in zip(r['numels'],r['rms']))/sum(r['numels'])
        assert math.isclose(expected,r['weighted_proxy_elr'],rel_tol=1e-12)
...
def main():
    from extract_weighted_reference import extract
    if hasattr(sys.stdout,'reconfigure'):sys.stdout.reconfigure(encoding='utf-8',errors='replace')
    def interrupted(signum,frame):raise KeyboardInterrupt(signum)
    signal.signal(signal.SIGTERM,interrupted)
    p=argparse.ArgumentParser(description='Extract new weighted proxy ELR references from existing baselines; run two fresh-start matching jobs concurrently.')
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            futures=[pool.submit(extract,root/f'baseline_{tag}',out/f'weighted_reference_{tag}.jsonl') for tag in tags]
            configs=[f.result() for f in futures]
        event(out,events,stage='references',event='both_weighted_references_ready')
        jobs=[]
        for i,(tag,cfg) in enumerate(zip(tags,configs)):
            cfg=dict(cfg);stage=out/f'weighted_matching_{tag}';stage.mkdir()
            ...
            cfg.update(role='matching',resume_dir='',reference_file=str(out/f'weighted_reference_{tag}.jsonl'),
            ...
            worker=str(HERE/f'train_weighted_{tag}_fresh.py')
        ...
        run_pair(jobs)
        atomic_json(out/'pipeline_complete.json',dict(completed=True,metric='weighted_proxy_elr',fresh_start=True,stages=[j[1].name for j in jobs]))
        print('WEIGHTED PROXY ELR MATCHING COMPLETE:',out)
