"""Extract parameter-count-weighted proxy ELR targets from one completed baseline stage of wte_mean_elr_pipeline."""
...
def extract(baseline,output):
    ...
    with source.open('rb') as inp,tmp.open('w',encoding='utf-8') as out:
        for raw in inp:
            ...
            weighted_elr=sum(k*lr/v for k,v in zip(numels,rms))/total_numel
            ...
            out.write(json.dumps(dict(update_step=rows,pre_update_step=rows-1,names=names,
                weighted_proxy_elr=weighted_elr,total_controlled_numel=total_numel,numels=numels,actual_mean_elr=actual,reference_block_lr=lr,
                definition='sum_i (numel_i/N) * block_lr/RMS(W_i); tied WTE once with block_lr numerator; gamma excluded; actual WTE LR is 2x'))+'\n')
