# Fresh-start parameter-count-weighted proxy ELR matching; config from --config.
# Warmup follows baseline; then switch WD and match sum_i (numel_i/N)*block_lr/RMS_i.
# Control block matrices plus tied WTE once; actual WTE LR is 2x; gamma excluded.
import os
import argparse
...
assert args.weight_decay == 0.1 and args.post_weight_decay == 0.0, 'Wrong config for this runner'
assert args.role == 'matching'
assert not args.resume_dir, 'Weighted proxy fresh-start runners never load checkpoints'
...
if master_process:
    ...
    with open(logfile, "w") as f:
        ...
        runtime_record = dict(
            ...
            planned_resume_step=0, matching_start_update=args.warmup_iters+1,
            control_metric='weighted_proxy_elr', actual_wte_lr_multiplier=2.0,
...
controlled_numels = [p.numel() for _, p in controlled_named]
...
if args.role == 'matching':
    with open(args.reference_file, encoding='utf-8') as f:
        for line in f:
            ...
            assert row['numels'] == controlled_numels, 'Reference tensor size mismatch'
            assert row['total_controlled_numel'] == sum(controlled_numels)
            step_id = row['update_step']
            assert step_id not in reference
            assert row['pre_update_step'] == step_id - 1
            assert math.isfinite(row['weighted_proxy_elr']) and row['weighted_proxy_elr'] > 0
            reference[step_id] = row
    assert set(reference) == set(range(1, args.num_iterations + 1))

@torch.no_grad()
def record_or_match_weighted_proxy_elr(update_step):
    ...
    total_numel = sum(controlled_numels)
    inverse_rms_weighted = sum(k/v for k, v in zip(controlled_numels, values))/total_numel
    if not math.isfinite(inverse_rms_weighted) or inverse_rms_weighted <= 0:
        raise RuntimeError('Invalid weighted inverse RMS')
    target = None
    if update_step > args.warmup_iters:
        target = reference[update_step]['weighted_proxy_elr']
        lr = target / inverse_rms_weighted
    ...
    weighted_proxy_elr = sum(k*lr/v for k, v in zip(controlled_numels, values))/total_numel
    error = None if target is None else abs(weighted_proxy_elr/target-1.0)
    ...
    if master_process:
        record = dict(update_step=update_step, pre_update_step=update_step-1, names=controlled_names,
                      ...
                      weighted_proxy_elr=weighted_proxy_elr, weighted_inverse_rms=inverse_rms_weighted, total_controlled_numel=total_numel, numels=controlled_numels, target_weighted_proxy_elr=target, control_metric='weighted_proxy_elr', relative_error=error, weight_decay=block['weight_decay'])
        with open(Path(args.output_dir) / 'weighted_proxy_elr_history.jsonl', 'a', encoding='utf-8') as f:
            f.write(json.dumps(record)+'\n')
...
for step in range(start_step, args.num_iterations + 1):
    ...
    update_step = step + 1
    record_or_match_weighted_proxy_elr(update_step)
