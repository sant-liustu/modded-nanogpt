...
# Purpose: rank-1 ELR AdamW consistency test at B=128, T=1024, 20400 steps.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Every controlled 2-D tensor, including tied wte/head, uses
# lr_i(t) = target_elr_i(t) * RMS(W_i(t)), where target_elr_i is read from a
# 73-tensor rank-1 target file. RMSNorm gamma remains on the AdamW WSD schedule.
# Weight decay intentionally stays fixed at 0.1 even when the source target had
# a different WD trajectory; this tests rank-1 schedule transfer across norms.
# Set RANK1_ELR_START_UPDATE=1001 to retain ordinary WD=0.1 AdamW WSD learning
# rates through update 1000, then hand control to a post-update-1000 target.
import os
import random
import sys
...
class Hyperparameters:
    ...
    rank1_elr_target_json : str = os.environ.get(
        'RANK1_ELR_TARGET_JSON',
        'experiments/rank1_elr_adamw_consistency/rank1_targets_from_wd0_wsd_reference_dense_preupdate.jsonl',
    )
    rank1_elr_start_update : int = int(os.environ.get('RANK1_ELR_START_UPDATE', '1'))
...
def load_rank1_elr_targets(path):
    ...
    if (
        ...
        raise ValueError(
            ...
            f'{declared_controller_start}, but RANK1_ELR_START_UPDATE='
...
def maybe_apply_rank1_elr_controller(update_step):
    if update_step < args.rank1_elr_start_update:
        # Match the ordinary AdamW WSD prefix exactly: LambdaLR's first update
        # uses get_lr(0), so update u uses get_lr(u - 1).  WD remains 0.1.
