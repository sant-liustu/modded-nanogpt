# Pure rank-1 residual ablation against the WD=0 WSD AdamW baseline.
# Updates 1..1000 exactly retain the baseline optimizer policy:
#   * WTE LR = 0.0036 * WSD multiplier, block LR = 0.0018 * multiplier;
#   * AdamW weight decay = 0 for all controlled matrices and RMSNorm gamma.
# Updates 1001..20400 retain WD=0 but replace the 73-tensor baseline ELR
# suffix by its uncentered best rank-one approximation:
#   lr_i(u) = E_rank1,i(u) * RMS(W_i^(u-1)).
# The tied WTE is one of the 73 target rows; no extra multiplier is applied.
import os
import random
import sys
...
class Hyperparameters:
    ...
    weight_decay : float = 0.0 # pure residual ablation: WD stays equal to the WD=0 reference throughout
    rank1_elr_target_json : str = 'experiments/rank1_elr_adamw_consistency/rank1_targets_from_wd0_wsd_reference_post1000_dense_preupdate.jsonl'
    rank1_elr_start_update : int = 1001
    rank1_elr_log_every : int = 4
    rank1_elr_detailed_log_every : int = 1000
...
if not 1 <= args.rank1_elr_start_update <= args.num_iterations:
    raise ValueError(
        f'rank1_elr_start_update must be in 1..{args.num_iterations}, '
        f'got {args.rank1_elr_start_update}'
    )
...
controlled_2d_named_parameters = [
    (canonical_parameter_name(name), p)
    for name, p in raw_model.named_parameters()
    if p.requires_grad and p.ndim == 2
]
rmsnorm_gamma_named_parameters = [
    (canonical_parameter_name(name), p)
    for name, p in raw_model.named_parameters()
    if p.requires_grad and p.ndim != 2
]
unexpected_non_2d = [
    (name, tuple(p.shape))
    for name, p in rmsnorm_gamma_named_parameters
    if not is_rmsnorm_gamma_name(name)
]
if unexpected_non_2d:
    raise RuntimeError(f'unexpected trainable non-2D parameters: {unexpected_non_2d}')
if len(controlled_2d_named_parameters) != 73:
    raise RuntimeError(
        f'expected 73 controlled 2-D tensors, found {len(controlled_2d_named_parameters)}'
    )

controlled_2d_parameters = [p for _, p in controlled_2d_named_parameters]
controlled_2d_names = [name for name, _ in controlled_2d_named_parameters]
embedding_controlled_indices = [
    index
    for index, name in enumerate(controlled_2d_names)
    if name == 'transformer.wte.weight'
]
if len(embedding_controlled_indices) != 1:
    raise RuntimeError(
        f'expected exactly one tied embedding tensor, found indices={embedding_controlled_indices}'
    )
embedding_controlled_index = embedding_controlled_indices[0]
rmsnorm_gamma_parameters = [p for _, p in rmsnorm_gamma_named_parameters]
controlled_optimizer_groups = [
    dict(
        params=[p],
        lr=1.0,  # overwritten by the rank-1 ELR controller before every optimizer step
        weight_decay=args.weight_decay,
        rank1_elr_name=name,
    )
    for name, p in controlled_2d_named_parameters
]
...
optimizers = [controlled_optimizer, gamma_optimizer]

def load_rank1_elr_targets(path):
    targets = {}
    declared_controller_start = None
    has_declared_controller_start = None
    with open(path, 'r', encoding='utf-8-sig') as f:
        for line_number, line in enumerate(f, start=1):
            ...
            if update_step in targets:
                raise ValueError(f'duplicate rank-1 ELR target for update_step={update_step}')
            record_has_start = 'rank1_controller_start_update' in record
            if has_declared_controller_start is None:
                has_declared_controller_start = record_has_start
            elif has_declared_controller_start != record_has_start:
                raise ValueError(
                    'rank-1 target must either declare rank1_controller_start_update on every row '
                    'or on no rows'
                )
            if record_has_start:
                record_start = int(record['rank1_controller_start_update'])
                if declared_controller_start is None:
                    declared_controller_start = record_start
                elif declared_controller_start != record_start:
                    raise ValueError(
                        'rank-1 target has inconsistent rank1_controller_start_update values'
                    )
            target_map = record['target_lr_over_tensor_norms']
            target_names = set(target_map)
            expected_names = set(controlled_2d_names)
            ...
            if missing_names or extra_names:
                raise ValueError(
                    f'rank-1 target at line {line_number} must contain exactly the 73 controlled tensors; '
                    f'missing={missing_names[:5]}, extra={extra_names[:5]}'
                )
            target_values = []
            for name in controlled_2d_names:
                target_elr = float(target_map[name])
                if not math.isfinite(target_elr) or target_elr <= 0.0:
                    raise ValueError(
                        f'invalid target ELR for {name!r} at line {line_number}: {target_elr}; '
                        'expected finite and > 0'
                    )
                target_values.append(target_elr)
            targets[update_step] = target_values
    expected_steps = set(range(1, args.num_iterations + 1))
    missing_steps = sorted(expected_steps - set(targets))
    extra_steps = sorted(set(targets) - expected_steps)
    if missing_steps or extra_steps:
        raise ValueError(
            'rank-1 ELR target must contain exactly one 73-tensor target for every update; '
            f'missing={missing_steps[:10]}, extra={extra_steps[:10]}'
        )
    if (
        declared_controller_start is not None
        and declared_controller_start != args.rank1_elr_start_update
    ):
        raise ValueError(
            'target declares rank1_controller_start_update='
            f'{declared_controller_start}, but configured rank1_elr_start_update='
            f'{args.rank1_elr_start_update}'
        )
    return targets

rank1_elr_targets = load_rank1_elr_targets(args.rank1_elr_target_json)

def should_log_rank1_elr(update_step):
    return (
        master_process
        and args.rank1_elr_log_every > 0
        and (
            update_step % args.rank1_elr_log_every == 0
            or update_step == 1
            or update_step == args.rank1_elr_start_update
            or update_step == args.num_iterations
        )
    )

@torch.no_grad()
def maybe_apply_rank1_elr_controller(update_step):
    if update_step < args.rank1_elr_start_update:
        # Replay the WD=0 reference prefix exactly at the optimizer-policy level:
        # LambdaLR's first update uses get_lr(0), so update u uses get_lr(u - 1).
        # args.weight_decay remains 0.0 for WTE and all block matrices.
        schedule_multiplier = get_lr(update_step - 1)
        embedding_lr = args.embed_learning_rate * schedule_multiplier
        block_lr = 0.5 * args.embed_learning_rate * schedule_multiplier
        for group in controlled_optimizer.param_groups:
            group['lr'] = (
                embedding_lr
                if group['rank1_elr_name'] == 'transformer.wte.weight'
                else block_lr
            )
        return

    target_elr_values = rank1_elr_targets[update_step]
    fro_norms = torch._foreach_norm(controlled_2d_parameters, 2)
    rms_values = torch.stack([
        fro_norm / math.sqrt(parameter.numel())
        for fro_norm, parameter in zip(fro_norms, controlled_2d_parameters)
    ])
    target_elr_tensor = torch.tensor(
        target_elr_values,
        device=device,
        dtype=rms_values.dtype,
    )
    learning_rates = (rms_values * target_elr_tensor).tolist()  # one host sync for all 73 tensors
    rms_values = rms_values.tolist()

    actual_elrs = []
    for group, learning_rate, rms_value in zip(
        controlled_optimizer.param_groups,
        learning_rates,
        rms_values,
    ):
        group['lr'] = float(learning_rate)
        actual_elrs.append(float(learning_rate) / float(rms_value))

    if should_log_rank1_elr(update_step):
        max_relative_error = max(
            abs(actual_elr - tensor_target_elr) / tensor_target_elr
            for actual_elr, tensor_target_elr in zip(actual_elrs, target_elr_values)
        )
        block_actual_elrs = [
            actual_elr
            for index, actual_elr in enumerate(actual_elrs)
            if index != embedding_controlled_index
        ]
        embedding_actual_elr = actual_elrs[embedding_controlled_index]
        record = dict(
            update_step=update_step,
            experiment_mode='wd0_baseline_prefix_wd0_rank1_suffix',
            prefix_weight_decay=0.0,
            rank1_suffix_weight_decay=0.0,
            target_elr_min=min(target_elr_values),
            target_elr_mean=sum(target_elr_values) / len(target_elr_values),
            target_elr_max=max(target_elr_values),
            target_embedding_elr=target_elr_values[embedding_controlled_index],
            target_file=args.rank1_elr_target_json,
            rank1_elr_start_update=args.rank1_elr_start_update,
            rank1_elr_active=True,
            controlled_tensor_count=len(controlled_2d_named_parameters),
            rms_min=min(rms_values),
            rms_mean=sum(rms_values) / len(rms_values),
            rms_max=max(rms_values),
            learning_rate_min=min(learning_rates),
            learning_rate_mean=sum(learning_rates) / len(learning_rates),
            learning_rate_max=max(learning_rates),
            actual_elr_min=min(actual_elrs),
            actual_elr_mean=sum(actual_elrs) / len(actual_elrs),
            actual_elr_max=max(actual_elrs),
            actual_block_elr_min=min(block_actual_elrs),
            actual_block_elr_mean=sum(block_actual_elrs) / len(block_actual_elrs),
            actual_block_elr_max=max(block_actual_elrs),
            actual_embedding_elr=embedding_actual_elr,
            max_relative_error=max_relative_error,
            weight_decay=args.weight_decay,
            weight_decay_compensated=False,
            elr_definition='learning_rate / parameter_rms',
        )
        if (
            args.rank1_elr_detailed_log_every > 0
            and (
                update_step % args.rank1_elr_detailed_log_every == 0
                or update_step == 1
                or update_step == args.rank1_elr_start_update
                or update_step == args.num_iterations
            )
        ):
            names = [name for name, _ in controlled_2d_named_parameters]
            record['target_tensor_elr'] = dict(zip(names, target_elr_values))
            record['tensor_rms'] = dict(zip(names, rms_values))
            record['tensor_learning_rates'] = dict(zip(names, learning_rates))
        with open(os.path.join(logdir, 'rank1_elr_history.jsonl'), 'a') as f:
...
def get_lr(it):
    ...
    # 3) linear warmdown
        ...
        return decay_ratio
gamma_scheduler = torch.optim.lr_scheduler.LambdaLR(gamma_optimizer, get_lr)
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    maybe_apply_rank1_elr_controller(update_step)
