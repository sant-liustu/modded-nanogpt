...
if master_process:
    ...
    with open(logfile, "w") as f:
        ...
        runtime_record = dict(
            ...
            control_metric='proxy_mean_elr', actual_wte_lr_multiplier=2.0,
...
if args.role == 'matching':
    with open(args.reference_file, encoding='utf-8') as f:
        for line in f:
            ...
            assert math.isfinite(row['proxy_mean_elr']) and row['proxy_mean_elr'] > 0
...
def record_or_match_mean_elr(update_step):
    ...
    inverse_sum = sum(1.0/v for v in values[:-1]) + 1.0/values[-1]
    target = None
    if update_step > args.warmup_iters:
        target = reference[update_step]['proxy_mean_elr']
    ...
    proxy = sum(lr/v for v in values)/len(values)
    error = None if target is None else abs(proxy/target-1.0)
    ...
    if master_process:
        record = dict(update_step=update_step, pre_update_step=update_step-1, names=controlled_names,
                      ...
                      proxy_mean_elr=proxy, target_proxy_mean_elr=target, control_metric='proxy_mean_elr', relative_error=error, weight_decay=block['weight_decay'])
