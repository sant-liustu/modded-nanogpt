# Copied from the successful total-F WD 0 -> 0.1 matching runner.
# Purpose: from update 1, match the literal WTE-inclusive 73-tensor mean ELR.
# WTE enters the statistic with its actual 2x block LR and receives that same LR in the update.
# Token budget matches the B=512, T=1024, 5100-step large-batch setup.
# Config: batch_size=128, device_batch_size=64, sequence_length=1024, num_iterations=20400, lr=0.0036, block_weight_decay=0.1 from initialization, seed=0
import os
import random
import sys
...
class Hyperparameters:
    ...
    weight_decay : float = 0.1 # fixed from initialization for block weights and tied wte/lm_head; RMSNorm gamma is excluded
    ...
    seed : int = 0
    mean_elr_reference_json : str = 'experiments/wte_inclusive_mean_elr_adamw_consistency/reference_wte_inclusive_73mean_elr_wd0_wsd_rep01.jsonl'
    mean_elr_control_start_update : int = 1 # match the 73-tensor mean coordinate from the first update
    mean_elr_match_log_every : int = 1
...
optimizer2_groups = [dict(params=block_weight_parameters, weight_decay=args.weight_decay, mean_elr_match_group=True)]
if rmsnorm_gamma_parameters:
    optimizer2_groups.append(dict(params=rmsnorm_gamma_parameters, weight_decay=0.0, mean_elr_match_group=False))
optimizer2 = torch.optim.AdamW(optimizer2_groups, lr=0.5*args.embed_learning_rate, betas=(0.9, 0.95),
                               fused=True)
optimizers = [optimizer1, optimizer2]

mean_elr_params = block_weight_parameters + [raw_model.transformer.wte.weight]
if len(mean_elr_params) != 73:
    raise RuntimeError(f'expected 72 block matrices plus tied WTE, found {len(mean_elr_params)} controlled tensors')

def load_mean_elr_targets(path):
    targets = {}
    with open(path, 'r', encoding='utf-8-sig') as f:
        for line_number, line in enumerate(f, start=1):
            if not line.strip():
                continue
            rec = json.loads(line)
            update_step = int(rec['update_step'])
            target_mean = float(rec['target_wte_inclusive_actual_mean_elr'])
            if update_step in targets:
                raise ValueError(f'duplicate actual-mean target for update_step={update_step}')
            if not math.isfinite(target_mean) or target_mean <= 0.0:
                raise ValueError(f'invalid actual-mean target at line {line_number}: {target_mean}')
            targets[update_step] = rec
    ...
    if missing_steps or extra_steps:
        raise ValueError(f'mean-ELR reference must cover every update; missing={missing_steps[:10]}, extra={extra_steps[:10]}')
    return targets

mean_elr_targets = load_mean_elr_targets(args.mean_elr_reference_json)

@torch.no_grad()
def current_wte_inclusive_mean_elr_terms():
    fro_norms = torch._foreach_norm(mean_elr_params, 2)
    rms_values = torch.stack([
        fro_norm.detach().float() / math.sqrt(parameter.numel())
        for fro_norm, parameter in zip(fro_norms, mean_elr_params)
    ])
    if not torch.isfinite(rms_values).all() or (rms_values <= 0.0).any():
        raise RuntimeError('non-finite or non-positive RMS in WTE-inclusive mean-ELR controller')
    inverse_rms = torch.reciprocal(rms_values)
    proxy_inverse_rms_sum = inverse_rms.sum().item()
    actual_inverse_rms_sum = (inverse_rms[:-1].sum() + 2.0 * inverse_rms[-1]).item()
    return rms_values, proxy_inverse_rms_sum, actual_inverse_rms_sum

def should_log_mean_elr_match(update_step):
    if not master_process or args.mean_elr_match_log_every <= 0:
        return False
    return (
        update_step % args.mean_elr_match_log_every == 0
        or update_step == args.mean_elr_control_start_update
        or update_step == args.num_iterations
    )

def maybe_apply_wte_inclusive_actual_mean_elr_controller(update_step):
    if update_step < args.mean_elr_control_start_update:
        return
    target = mean_elr_targets[update_step]
    target_mean_elr = float(target['target_wte_inclusive_actual_mean_elr'])
    rms_values, proxy_inverse_rms_sum, actual_inverse_rms_sum = current_wte_inclusive_mean_elr_terms()
    adjusted_lr = len(mean_elr_params) * target_mean_elr / actual_inverse_rms_sum
    ...
    for group in optimizer2.param_groups:
        if not group.get('mean_elr_match_group', False):
            continue
        group['lr'] = adjusted_lr
        group['weight_decay'] = target_weight_decay
    realized_proxy_mean_elr = adjusted_lr * proxy_inverse_rms_sum / len(mean_elr_params)
    realized_actual_mean_elr = adjusted_lr * actual_inverse_rms_sum / len(mean_elr_params)
    control_relative_error = abs(realized_actual_mean_elr - target_mean_elr) / target_mean_elr
    if control_relative_error > 1e-6:
        raise RuntimeError(f'actual mean-ELR control error too large: {control_relative_error}')
    if should_log_mean_elr_match(update_step):
        record = dict(
            update_step=update_step,
            pre_update_step=update_step - 1,
            controller='wte_inclusive_actual_mean_elr',
            controlled_tensor_count=len(mean_elr_params),
            mean_definition=target['actual_mean_definition'],
            target_wte_inclusive_proxy_mean_elr=float(target['target_wte_inclusive_proxy_mean_elr']),
            target_wte_inclusive_actual_mean_elr=target_mean_elr,
            ...
            proxy_inverse_rms_sum=proxy_inverse_rms_sum,
            actual_inverse_rms_sum=actual_inverse_rms_sum,
            realized_wte_inclusive_proxy_mean_elr=realized_proxy_mean_elr,
            realized_wte_inclusive_actual_mean_elr=realized_actual_mean_elr,
            control_relative_error=control_relative_error,
            actual_wte_elr=adjusted_embed_lr / rms_values[-1].item(),
            mean_elr_control_start_update=args.mean_elr_control_start_update,
            weight_decay_mode='wd0p1_from_initialization',
            target_weight_decay=target_weight_decay,
            optimizer1_weight_decay=target_weight_decay,
            optimizer2_weight_decay=target_weight_decay,
            norm_scope='72 transformer block matrices plus tied WTE; per-tensor RMS',
        )
        with open(os.path.join(logdir, 'wte_inclusive_mean_elr_history.jsonl'), 'a') as f:
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    maybe_apply_wte_inclusive_actual_mean_elr_controller(update_step)
