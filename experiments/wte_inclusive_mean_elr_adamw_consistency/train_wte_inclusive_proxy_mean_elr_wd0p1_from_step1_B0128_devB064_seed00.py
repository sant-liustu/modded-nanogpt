...
# Purpose: from update 1, match a WTE-inclusive 73-tensor proxy mean ELR.
# WTE enters the statistic with base block LR, but its actual optimizer LR is 2x.
...
def load_mean_elr_targets(path):
    ...
    with open(path, 'r', encoding='utf-8-sig') as f:
        for line_number, line in enumerate(f, start=1):
            ...
            target_mean = float(rec['target_wte_inclusive_proxy_mean_elr'])
            if update_step in targets:
                raise ValueError(f'duplicate proxy-mean target for update_step={update_step}')
            if not math.isfinite(target_mean) or target_mean <= 0.0:
                raise ValueError(f'invalid proxy-mean target at line {line_number}: {target_mean}')
...
def maybe_apply_wte_inclusive_proxy_mean_elr_controller(update_step):
    if update_step < args.mean_elr_control_start_update:
        return
    target = mean_elr_targets[update_step]
    target_mean_elr = float(target['target_wte_inclusive_proxy_mean_elr'])
    rms_values, proxy_inverse_rms_sum, actual_inverse_rms_sum = current_wte_inclusive_mean_elr_terms()
    adjusted_lr = len(mean_elr_params) * target_mean_elr / proxy_inverse_rms_sum
    ...
    control_relative_error = abs(realized_proxy_mean_elr - target_mean_elr) / target_mean_elr
    if control_relative_error > 1e-6:
        raise RuntimeError(f'proxy mean-ELR control error too large: {control_relative_error}')
    if should_log_mean_elr_match(update_step):
        record = dict(
            update_step=update_step,
            pre_update_step=update_step - 1,
            controller='wte_inclusive_proxy_mean_elr',
            controlled_tensor_count=len(mean_elr_params),
            mean_definition=target['proxy_mean_definition'],
            target_wte_inclusive_proxy_mean_elr=target_mean_elr,
            target_wte_inclusive_actual_mean_elr=float(target['target_wte_inclusive_actual_mean_elr']),
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    maybe_apply_wte_inclusive_proxy_mean_elr_controller(update_step)
