# Experiment 2B: Adam hard-norm stress-test arm with the heterogeneous RMS-ELR profile.
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # MuonH-compatible initialization for every block matrix:
        # W_ij ~ N(0, 1 / d_in).
...
class Hyperparameters:
    ...
    seed : int = 0
    norm_control_config : str = 'experiments/norm_control_schedule_collapse/hardnorm_assignment_pertensor_B_seed20260904.json'
...
def load_per_tensor_rms_elr_targets(path):
    if not path:
        raise ValueError(
            "per_tensor_elr_file is required; pass --per_tensor_elr_file=<RMS-ELR.jsonl[.gz]>"
        )
    rows = []
    ...
    with opener(path, mode, encoding='utf-8') as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            if update_step != expected_step:
                raise RuntimeError(
                    f"expected RMS ELR update_step {expected_step}, got {update_step} "
                    f"at {path}:{line_number}"
                )
            declared_norm_type = record.get('elr_norm_type', record.get('norm_type'))
            norm_type = None if declared_norm_type is None else str(declared_norm_type).lower()
            norm_scope = str(record.get('norm_scope', '')).lower()
            if norm_type not in (None, 'rms') or 'frobenius' in norm_scope or 'fro_norm' in norm_scope:
                raise ValueError(
                    f"{path}:{line_number} is not an RMS-ELR record: "
                    f"norm_type={norm_type!r}, norm_scope={norm_scope!r}"
                )
            if 'target_lr_over_tensor_rms' in record:
                target_field = 'target_lr_over_tensor_rms'
            elif 'target_lr_over_tensor_norms' in record and norm_type == 'rms':
                # Allow an older generic field name only when the file explicitly
                # declares that its denominator is RMS.
                target_field = 'target_lr_over_tensor_norms'
            elif 'target_lr_over_tensor_norms' in record:
                raise ValueError(
                    f"{path}:{line_number} uses ambiguous target_lr_over_tensor_norms; "
                    "declare norm_type='rms' or rename it to target_lr_over_tensor_rms"
                )
            else:
                raise ValueError(
                    f"missing target_lr_over_tensor_rms at {path}:{line_number}"
                )
            targets = {
                str(name): float(value)
                for name, value in record[target_field].items()
            }
            bad = {
                name: value
            ...
            if bad:
                raise ValueError(
                    f"RMS ELR targets must be finite and positive at {path}:{line_number}; "
                    f"first bad entries: {list(bad.items())[:5]}"
                )
            rows.append(targets)
    if len(rows) != args.num_iterations:
        raise RuntimeError(
            f"expected {args.num_iterations} RMS ELR rows in {path}, got {len(rows)}"
        )
    return rows

per_tensor_rms_elr_targets = load_per_tensor_rms_elr_targets(args.per_tensor_elr_file)
...
def load_norm_control_config(path):
    if not path:
        return dict(enabled=False, mode='disabled', assignments={}, eps=1e-12, log_every=1)
    ...
    mode = spec.get('mode')
    if mode != 'hard_schedule_from_initial_rms':
        raise ValueError(
            "hard-norm stress scripts require mode='hard_schedule_from_initial_rms'"
        )
    assignments = spec.get('assignments')
    if not isinstance(assignments, dict) or not assignments:
        raise ValueError("hard norm-control config requires a non-empty assignments object")
    allowed_schedules = {'constant', 'linear_up', 'linear_down', 'cosine_cycle'}
    invalid_assignments = {
        name: schedule
    ...
    if invalid_assignments:
        raise ValueError(f"unknown hard norm schedules: {list(invalid_assignments.items())[:5]}")
    expected_count = int(spec.get('controlled_tensor_count', len(assignments)))
    if expected_count != len(assignments):
        raise ValueError(
            f"controlled_tensor_count={expected_count} does not match {len(assignments)} assignments"
        )
    return dict(
        enabled=True,
        mode=mode,
        assignments={str(name): str(schedule) for name, schedule in assignments.items()},
        eps=float(spec.get('eps', 1e-12)),
        log_every=int(spec.get('log_every', 1)),
        assignment_seed=spec.get('assignment_seed'),
        selection_method=spec.get('selection_method'),
        schedule_definitions=spec.get('hard_norm_schedules'),
...
def build_norm_control_state(raw_model, path):
    ...
    named_parameters = {
        canonical_param_name(name): (name, p)
        for name, p in raw_model.named_parameters()
    }
    configured_names = set(spec['assignments'])
    available_names = set(named_parameters)
    missing = sorted(configured_names - available_names)
    if missing:
        raise ValueError(f"hard norm-control assignments matched no parameters: {missing[:5]}")
    controlled = []
    for canonical_name in sorted(configured_names):
        name, p = named_parameters[canonical_name]
        ...
        controlled.append(dict(
            name=name,
            param=p,
            schedule=spec['assignments'][canonical_name],
            initial_rms=None,
            captured=False,
        ))
    return dict(
        ...
        schedule_definitions=spec['schedule_definitions'],
        coordinate=spec['coordinate'],
    )

def hard_norm_schedule_ratio(schedule, control_step):
    if control_step < 0 or control_step > args.num_iterations:
        raise RuntimeError(f"invalid hard norm control step: {control_step}")
    x = control_step / args.num_iterations
    if schedule == 'constant':
        ...
        ratio = 1.0 - 0.5 * x
    elif schedule == 'cosine_cycle':
        ratio = 1.0 + 0.5 * math.sin(2.0 * math.pi * x)
    else:
        raise ValueError(f"unknown hard norm schedule: {schedule}")
    if not math.isfinite(ratio) or ratio <= 0.0:
        raise RuntimeError(f"invalid hard norm ratio for {schedule} at x={x}: {ratio}")
    return x, ratio
...
def apply_rms_norm_control(norm_control_state, step, event):
    ...
    try:
        for entry in norm_control_state['params']:
            ...
            captured = event == 'initial'
            if captured:
                if step != 0:
                    raise RuntimeError('initial hard norm capture must occur at step 0')
                initial_rms = rms_before.item()
                if not math.isfinite(initial_rms) or initial_rms <= norm_control_state['eps']:
                    raise RuntimeError(f"invalid initial RMS for {entry['name']}: {initial_rms}")
                entry['initial_rms'] = initial_rms
                entry['captured'] = True
            if entry['initial_rms'] is None:
                raise RuntimeError(f"missing step-0 RMS capture for {entry['name']}")
            x, ratio = hard_norm_schedule_ratio(entry['schedule'], step)
            target_rms = entry['initial_rms'] * ratio
            projected = not captured
            if projected:
            ...
            if f is not None:
                ...
                f.write(json.dumps(dict(
                    ...
                    schedule_coordinate=x,
                    schedule_ratio=ratio,
                    initial_rms=entry['initial_rms'],
...
def write_norm_control_targets(norm_control_state):
    ...
    records = []
    for entry in norm_control_state['params']:
        if entry['initial_rms'] is not None:
            records.append(dict(
                name=entry['name'],
                schedule=entry['schedule'],
                initial_rms=entry['initial_rms'],
...
def write_norm_control_metadata(norm_control_state):
    ...
    metadata = dict(
        ...
        assignment_seed=norm_control_state['assignment_seed'],
        selection_method=norm_control_state['selection_method'],
        schedule_definitions=norm_control_state['schedule_definitions'],
        coordinate=norm_control_state['coordinate'],
...
controlled_parameters_by_name = {
    entry['name']: entry['param']
    for entry in norm_control_state['params']
}
...
controlled_block_parameters_by_name = {
    name: p
    for name, p in block_named_parameters
    if id(p) in controlled_param_ids
}
...
expected_elr_names = set(controlled_parameters_by_name)
if len(expected_elr_names) != 73 or 'transformer.wte.weight' not in expected_elr_names:
    raise RuntimeError(
        "hard-norm ELR control requires exactly 72 block matrices plus "
        f"transformer.wte.weight; got {len(expected_elr_names)} tensors"
    )
for row_index, targets in enumerate(per_tensor_rms_elr_targets, 1):
    target_names = set(targets)
    if target_names != expected_elr_names:
        missing = sorted(expected_elr_names - target_names)
        extra = sorted(target_names - expected_elr_names)
        raise RuntimeError(
            f"RMS ELR names mismatch at update_step={row_index}; "
            f"missing={missing[:5]} extra={extra[:5]}"
        )

# init the optimizer(s)
tied_embedding_parameter = raw_model.transformer.wte.weight
if controlled_parameters_by_name['transformer.wte.weight'] is not tied_embedding_parameter:
    raise RuntimeError('norm-control tied embedding does not alias transformer.wte.weight')
optimizer1 = torch.optim.AdamW([
    dict(
        params=[tied_embedding_parameter],
        lr=0.0,
        weight_decay=0.0,
        elr_controlled=True,
        tensor_name='transformer.wte.weight',
    )
], lr=0.0, betas=(0.9, 0.95), fused=True)
optimizer2_groups = []
if uncontrolled_block_parameters:
    optimizer2_groups.append(dict(
        params=uncontrolled_block_parameters,
        lr=0.5*args.embed_learning_rate,
        weight_decay=args.weight_decay,
        elr_controlled=False,
        group_role='uncontrolled_block',
    ))
for name in sorted(controlled_block_parameters_by_name):
    optimizer2_groups.append(dict(
        params=[controlled_block_parameters_by_name[name]],
        lr=0.0,
        weight_decay=0.0,
        elr_controlled=True,
        tensor_name=name,
...
if rmsnorm_gamma_parameters:
    optimizer2_groups.append(dict(
        params=rmsnorm_gamma_parameters,
        lr=0.5*args.embed_learning_rate,
        weight_decay=0.0,
        elr_controlled=False,
        group_role='rmsnorm_gamma',
    ))
optimizer2 = torch.optim.AdamW(optimizer2_groups, lr=0.5*args.embed_learning_rate, betas=(0.9, 0.95),
                               fused=True)
optimizers = [optimizer1, optimizer2]

elr_param_groups_by_name = {}
for optimizer in optimizers:
    for group in optimizer.param_groups:
        if not group.get('elr_controlled', False):
            continue
        name = group['tensor_name']
        if name in elr_param_groups_by_name:
            raise RuntimeError(f'duplicate ELR-controlled optimizer group for {name}')
        if len(group['params']) != 1 or group['params'][0] is not controlled_parameters_by_name[name]:
            raise RuntimeError(f'invalid one-tensor optimizer group for {name}')
        elr_param_groups_by_name[name] = group
if set(elr_param_groups_by_name) != expected_elr_names:
    raise RuntimeError('ELR-controlled optimizer groups do not exactly cover controlled tensors')

def apply_per_tensor_rms_elr(update_step):
    if update_step < 1 or update_step > args.num_iterations:
        raise RuntimeError(f'invalid RMS ELR update_step={update_step}')
    targets = per_tensor_rms_elr_targets[update_step - 1]
    current_rms_by_name = {}
    actual_lr_by_name = {}
    actual_elr_by_name = {}
    for name in sorted(expected_elr_names):
        parameter = controlled_parameters_by_name[name]
        current_rms = parameter.detach().float().square().mean().sqrt().item()
        if not math.isfinite(current_rms) or current_rms <= norm_control_state['eps']:
            raise RuntimeError(f'cannot apply positive RMS ELR to invalid RMS for {name}: {current_rms}')
        target_elr = targets[name]
        actual_lr = target_elr * current_rms
        elr_param_groups_by_name[name]['lr'] = actual_lr
        current_rms_by_name[name] = current_rms
        actual_lr_by_name[name] = actual_lr
        actual_elr_by_name[name] = actual_lr / current_rms
    record = dict(
        update_step=update_step,
        norm_type='rms',
        target_lr_over_tensor_rms=targets,
        current_tensor_rms=current_rms_by_name,
        actual_lr_by_tensor=actual_lr_by_name,
        actual_lr_over_tensor_rms=actual_elr_by_name,
    )
    should_log = (
        master_process
        and args.per_tensor_elr_log_every > 0
        and (
            update_step % args.per_tensor_elr_log_every == 0
            or update_step == 1
            or update_step == args.num_iterations
        )
    )
    if should_log:
        with open(os.path.join(logdir, 'per_tensor_elr_history.jsonl'), 'a') as f:
            f.write(json.dumps(record) + '\n')
    return record

def write_per_tensor_elr_metadata():
    if not master_process:
        return
    metadata = dict(
        enabled=True,
        norm_type='rms',
        formula='lr_i(t) = target_elr_i(t) * rms(W_i(t))',
        target_field='target_lr_over_tensor_rms',
        source_file=args.per_tensor_elr_file,
        controlled_parameter_count=len(expected_elr_names),
        controlled_parameters=sorted(expected_elr_names),
        optimizer='AdamW with weight_decay=0 (Adam-equivalent update)',
        weight_decay=0.0,
        projection='post_step_to_assigned_hard_rms_trajectory',
    )
    with open(os.path.join(logdir, 'per_tensor_elr_metadata.json'), 'w') as f:
        json.dump(metadata, f, indent=2)
...
optimizer2_lr_lambdas = []
for group in optimizer2.param_groups:
    if group.get('elr_controlled', False):
        optimizer2_lr_lambdas.append(lambda _: 1.0)
    elif group.get('group_role') == 'rmsnorm_gamma':
        optimizer2_lr_lambdas.append(get_wsd_lr)
    else:
        optimizer2_lr_lambdas.append(get_lr)
schedulers = [
    torch.optim.lr_scheduler.LambdaLR(optimizer1, lambda _: 1.0),
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    per_tensor_elr_record = apply_per_tensor_rms_elr(update_step)
