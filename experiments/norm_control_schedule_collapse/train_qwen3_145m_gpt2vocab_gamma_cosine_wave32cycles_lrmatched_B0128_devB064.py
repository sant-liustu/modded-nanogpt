# Qwen3 thirty-two-cycle cosine-wave norm-control experiment.
# Capture every unique 2D tensor RMS after update 1000, then oscillate it over [0.5x, 1.5x].
# Match every controlled-matrix LR to the same period-625 ratio; keep RMSNorm gamma on base WSD.
...
#         weight_decay=0.0, controlled_2d_tensors=85, seed=0
import os
import random
import sys
...
# -----------------------------------------------------------------------------
# The Qwen3 model definitions are inlined above so this experiment is self-contained.
...
class Hyperparameters:
    ...
    norm_control_start_step : int = 1000 # capture reference RMS after this optimizer update
    ...
    tensor_norm_every : int = 1 # log tensor norm history every step; 0 disables
    adamw_update_norm_every : int = 1 # log AdamW effective update norms every optimizer step; 0 disables
    activation_probe_every : int = 0 # every how many steps to log fixed-probe activation RMS ratios? 0 disables
    spectral_norm_estimate_enabled : int = 0 # do not estimate or log 2D spectral norms
...
NORM_CONTROL_SCHEDULE = 'cosine_wave_period625_amp0p5'
...
def build_all_matrix_norm_control_state(model):
    ...
    for name, parameter in model.named_parameters():
        ...
        if parameter_id in seen_ids:
            raise RuntimeError(f"duplicate 2D norm-control parameter: {canonical_param_name(name)}")
    ...
    expected_count = qwen3_config.num_hidden_layers * 7 + 1
    if len(controlled) != expected_count:
        raise RuntimeError(
            f"expected {expected_count} unique 2D norm-control tensors, found {len(controlled)}"
        )

    controlled_ids = {id(entry['param']) for entry in controlled}
    expected_ids = {id(parameter) for parameter in lm_head_parameters + block_weight_parameters}
    if controlled_ids != expected_ids:
        missing = [
            canonical_param_name(name)
            for name, parameter in model.named_parameters()
            if id(parameter) in expected_ids - controlled_ids
        ]
        unexpected = [entry['name'] for entry in controlled if id(entry['param']) not in expected_ids]
        raise RuntimeError(
            f"2D norm-control routing mismatch: missing={missing}, unexpected={unexpected}"
        )
