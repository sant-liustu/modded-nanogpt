# KDA--full-attention 3:1 norm-control schedule: cosine-wave RMS ratio.
# Delayed-captured KDA QKVO RMS control, WSD LR matched by the same ratio, B=128, 20400 optimizer steps.
import os
import random
import sys
...
# FLA is intentionally optional at import time so this script can be syntax-checked
# locally before the CUDA training environment installs it.
try:
...
class KDAAttention(nn.Module):
    """Adapter giving FLA Kimi Delta Attention the existing x -> x interface."""

    def __init__(self, config, layer_idx):
        super().__init__()
        if KimiDeltaAttention is None:
            raise ImportError(
                "KDAAttention requires flash-linear-attention on the CUDA training "
                "environment. Install it with: pip install \"flash-linear-attention[cuda]\""
            )
        ...
        # Match the existing residual-writing attention projection initialization.
        kda_output_std = config.init_std / math.sqrt(2 * config.n_layer)
        torch.nn.init.normal_(self.kda.o_proj.weight, mean=0.0, std=kda_output_std)

    def forward(self, x):
        y, _, _ = self.kda(hidden_states=x, use_cache=False)
        return y
...
class Block(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        super().__init__()
        # One-indexed layers 4, 8, and 12 retain the original full attention.
        self.attn = (
            CausalSelfAttention(config)
            if (layer_idx + 1) % 4 == 0
            else KDAAttention(config, layer_idx)
        )
        self.mlp = MLP(config)
...
class Hyperparameters:
    ...
    seed : int = 0
    norm_control_config : str = 'experiments/norm_control_schedule_collapse/delayed_constant_all_matrices_start1000_kda3_full1_qkvo.json' # KDA QKVO projection-only per-tensor RMS norm control
...
def write_norm_control_metadata(norm_control_state):
    ...
    metadata = dict(
        ...
        kda_dynamics_parameters=[
            canonical_param_name(name) for name, _ in kda_dynamics_named_parameters
        ],
...
def is_rmsnorm_gamma_name(name):
    ...
    return (
        ...
        or canonical_name.endswith('.kda.o_norm.weight')
...
kda_dynamics_named_parameters = [
    (name, p)
    for name, p in block_named_parameters
    if '.attn.kda.' in canonical_param_name(name) and id(p) not in controlled_param_ids
]
kda_dynamics_param_ids = {id(p) for _, p in kda_dynamics_named_parameters}
uncontrolled_block_parameters = [
    p
    for _, p in block_named_parameters
    if id(p) not in controlled_param_ids and id(p) not in kda_dynamics_param_ids
]
...
optimizer2_groups = []
optimizer2_group_schedule_types = []
if uncontrolled_block_parameters:
    ...
    optimizer2_group_schedule_types.append('matched')
if controlled_block_parameters:
    optimizer2_groups.append(dict(params=controlled_block_parameters, weight_decay=0.0))
    optimizer2_group_schedule_types.append('matched')
if kda_dynamics_named_parameters:
    optimizer2_groups.append(dict(
        params=[p for _, p in kda_dynamics_named_parameters],
        weight_decay=0.0,
    ))
    optimizer2_group_schedule_types.append('wsd')
...
optimizer2_lr_lambdas = [
    get_lr if schedule_type == 'matched' else get_wsd_lr
    for schedule_type in optimizer2_group_schedule_types
]
schedulers = [
