# Width-2560 all-AdamW learnable-gamma cosine-wave norm-control experiment.
# Delayed capture at step 750; RMS target ratio oscillates around 1.0 and block LR is ratio-matched.
import os
import random
import sys
...
class GPTConfig:
    ...
    n_layer : int = 12
    n_head : int = 20 # preserve head dimension 128 at width 2560
    n_embd : int = 2560
    init_std : float = 0.02
...
class Hyperparameters:
    # data hyperparams
    input_bin : str = 'data/fineweb40B/fineweb_train_*.bin' # input .bin to train on
    input_val_bin : str = 'data/fineweb40B/fineweb_val_*.bin' # input .bin to eval validation loss on
    ...
    val_tokens : int = 9175040 # 10 validation micro-steps; divisible by 8 * 112 * 1024
    save_every : int = 0 # every how many steps to save the checkpoint? 0 for only at the end
    compile_model : int = 1 # compile the model with torch.compile
    tensor_norm_every : int = 100 # practical monitoring cadence for the 1.07B-parameter model
    adamw_update_norm_every : int = 100 # practical update-norm cadence for the 1.07B-parameter model
...
num_vocab = 50304
model = GPT(GPTConfig(vocab_size=num_vocab, n_layer=12, n_head=20, n_embd=2560))
model = model.cuda()
...
optimizer2 = torch.optim.AdamW(optimizer2_groups, lr=args.block_learning_rate, betas=(0.9, 0.95),
                               fused=True)
...
if master_process:
    ...
    grad_clip_logfile = os.path.join(logdir, 'grad_clip_history.jsonl')
    # create the log file
...
for step in range(args.num_iterations + 1):
    ...
    if args.grad_clip > 0:
        total_grad_norm = torch.nn.utils.clip_grad_norm_(
            model.parameters(), max_norm=args.grad_clip, norm_type=2.0,
    ...
    if master_process:
        grad_norm_value = total_grad_norm.item() if total_grad_norm is not None else None
        with open(grad_clip_logfile, 'a') as f:
            f.write(json.dumps(dict(
                step=update_step,
                grad_clip=args.grad_clip,
                total_grad_norm=grad_norm_value,
                clipped=(grad_norm_value is not None and grad_norm_value > args.grad_clip),
            )) + '\n')
