...
# Base LR uses 1000-step warmup followed by linear decay to zero.
import os
import random
import sys
...
class Hyperparameters:
    ...
    decay_iters : int = 19400 # post-warmup iterations over which base LR linearly decays to zero
...
# base learning-rate scheduler (linear warmup followed by linear decay to zero)
...
def get_base_lr(it):
    ...
    assert args.decay_iters == args.num_iterations - args.warmup_iters
    decay_step = it - args.warmup_iters
    return max(0.0, 1.0 - decay_step / args.decay_iters)
