# Muon cosine-wave norm-control schedule collapse experiment.
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group['params']):
                ...
                if i % self.world_size == self.rank:
                    ...
                    g *= max(g.size(0), g.size(1))**0.5 # scale update RMS to approximately one
...
class Hyperparameters:
    ...
    muon_learning_rate : float = 0.00036 # corrected Muon recipe: 0.1 * embed_learning_rate
...
if non_matrix_block_parameters:
    raise RuntimeError(f"Muon only accepts 2D transformer-block parameters: {non_matrix_block_parameters}")
...
if args.weight_decay != 0:
    raise RuntimeError("this Muon norm-control experiment requires weight_decay=0")
optimizer2_groups = []
...
optimizer2 = Muon(optimizer2_groups, lr=args.muon_learning_rate, momentum=0.95,
...
def should_log_muon_update_norms(update_step):
    if not master_process or args.muon_update_norm_every <= 0:
        return False
    return update_step % args.muon_update_norm_every == 0 or update_step == args.num_iterations

def maybe_capture_muon_update_state(update_step):
    if not should_log_muon_update_norms(update_step):
        return None
...
def muon_update_tensor(tensor, tensor_before, param_hparams, step, name):
    lr = param_hparams['lr']
    if lr == 0:
        raise RuntimeError(f"cannot infer Muon update for {name} at step {step}: lr is 0")
...
def muon_update_norm_record(step, name, tensor, muon_update, param_hparams, spectral_norm_estimate=None):
    lr = param_hparams['lr']
    weight_decay = param_hparams['weight_decay']
    record = dict(
        step=step,
...
def maybe_log_muon_update_norms(update_step, update_state):
    if update_state is None:
        return
    history_path = os.path.join(logdir, 'muon_update_norm_history.jsonl')
    ...
    with torch.no_grad(), open(history_path, 'a') as f:
        for name, tensor in raw_model.named_parameters():
            ...
            muon_update = muon_update_tensor(tensor, tensor_before, hparams[param_id], update_step, name)
            if args.spectral_norm_estimate_enabled > 0 and muon_update.ndim == 2:
                pending_2d_updates.append((name, muon_update))
                pending_records.append((name, tensor, muon_update, hparams[param_id]))
            else:
                record = muon_update_norm_record(update_step, name, tensor, muon_update, hparams[param_id])
                f.write(json.dumps(record) + '\n')
                del record
            del tensor_before
        spectral_estimates = spectral_norm_estimates_by_name(pending_2d_updates)
        for name, tensor, muon_update, param_hparams in pending_records:
            record = muon_update_norm_record(
                update_step,
                name,
                tensor,
                muon_update,
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    muon_update_state = maybe_capture_muon_update_state(update_step)
    for opt in optimizers:
        opt.step()
    maybe_log_muon_update_norms(update_step, muon_update_state)
