# AdamW weight-decay arm for the per-tensor ELR-govern experiment.
# MuonH-compatible block initialization, no norm projection, WD=0.1.
# Before every update: lr_i(t) = target_ELR_i(t) * RMS(W_i(t)).
import os
import random
import sys
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # MuonH-compatible initialization: W_ij ~ N(0, 1 / d_in).
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Same MuonH-compatible initialization for both MLP matrices.
...
class Hyperparameters:
    ...
    weight_decay : float = 0.1 # fixed AdamW decay for all 73 controlled 2-D tensors
    per_tensor_elr_file : str = '' # required RMS-ELR JSONL/JSONL.GZ with target_lr_over_tensor_norms
    per_tensor_elr_log_every : int = 4
    per_tensor_elr_detailed_log_every : int = 1000
    ...
    seed : int = 0
    norm_control_config : str = '' # explicitly disabled: this arm never projects parameter norms
...
if norm_control_state['enabled']:
    raise RuntimeError('norm projection must remain disabled in the WD=0.1 experiment arm')
...
# Every controlled 2-D tensor needs its own ordinary AdamW parameter group so
# the external file can prescribe a different RMS-normalized ELR for it.
controlled_2d_named_parameters = [
    (canonical_param_name(name), p)
    for name, p in raw_model.named_parameters()
    if p.requires_grad and p.ndim == 2
]
controlled_names = [name for name, _ in controlled_2d_named_parameters]
if len(controlled_names) != 73 or len(set(controlled_names)) != 73:
    raise RuntimeError(
        f'expected 73 unique controlled 2-D tensors, got {len(controlled_names)} '
        f'entries and {len(set(controlled_names))} unique names'
    )
unexpected_gamma_parameters = [
    (canonical_param_name(name), tuple(p.shape))
    for name, p in raw_model.named_parameters()
    if p.requires_grad and p.ndim != 2 and not is_rmsnorm_gamma_name(name)
]
if unexpected_gamma_parameters:
    raise RuntimeError(f'unexpected trainable non-2D parameters: {unexpected_gamma_parameters}')
...
controlled_optimizer = torch.optim.AdamW(
    [
        dict(
            params=[p],
            lr=1.0,  # overwritten by the per-tensor RMS-ELR controller before every step
            weight_decay=args.weight_decay,
            per_tensor_elr_name=name,
        )
...
def load_per_tensor_rms_elr_targets(path):
    if not path:
        raise ValueError(
            'per_tensor_elr_file is required; pass '
            '--per_tensor_elr_file=<RMS-ELR.jsonl[.gz]>'
        )
    rows = []
    expected_names = set(controlled_names)
    opener = gzip.open if path.endswith('.gz') else open
    mode = 'rt' if path.endswith('.gz') else 'r'
    with opener(path, mode, encoding='utf-8-sig') as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            norm_type = str(record.get('elr_norm_type', record.get('norm_type', 'rms'))).lower()
            norm_scope = str(record.get('norm_scope', '')).lower()
            if norm_type != 'rms' or 'frobenius' in norm_scope or 'fro_norm' in norm_scope:
                raise ValueError(
                    f'{path}:{line_number} is not an RMS-ELR record: '
                    f'norm_type={norm_type!r}, norm_scope={norm_scope!r}'
                )
            raw_targets = record.get('target_lr_over_tensor_norms')
            if not isinstance(raw_targets, dict):
                raise ValueError(
                    f'{path}:{line_number} must contain target_lr_over_tensor_norms'
                )
            if set(raw_targets) != expected_names:
                missing = sorted(expected_names - set(raw_targets))
                extra = sorted(set(raw_targets) - expected_names)
                raise ValueError(
                    f'RMS-ELR tensor names mismatch at update_step={update_step}; '
                    f'missing={missing[:5]}, extra={extra[:5]}'
                )
            targets = {name: float(raw_targets[name]) for name in controlled_names}
            invalid = [
            ...
            if invalid:
                raise ValueError(
                    f'non-finite or negative RMS-ELR targets at update_step={update_step}: '
                    f'{invalid[:5]}'
                )
            rows.append(targets)
...
per_tensor_rms_elr_targets = load_per_tensor_rms_elr_targets(
    args.per_tensor_elr_file
)

def should_log_per_tensor_elr(update_step):
    return (
        master_process
...
@torch.no_grad()
def maybe_apply_per_tensor_rms_elr_controller(update_step):
    targets = per_tensor_rms_elr_targets[update_step - 1]
    fro_norms = torch._foreach_norm(controlled_2d_parameters, 2)
    rms_tensor = torch.stack([
        fro_norm / math.sqrt(parameter.numel())
        for fro_norm, parameter in zip(fro_norms, controlled_2d_parameters)
    ])
    rms_values = rms_tensor.tolist()  # one host synchronization for all 73 tensors
    target_values = [targets[name] for name in controlled_names]
    learning_rates = [
        target_elr * rms_value
        for target_elr, rms_value in zip(target_values, rms_values)
    ]
    actual_elrs = []
    for group, learning_rate, rms_value in zip(
        ...
        if rms_value <= 0.0:
            raise RuntimeError(
                f"controlled tensor {group['per_tensor_elr_name']} has non-positive RMS"
            )
        group['lr'] = float(learning_rate)
        actual_elrs.append(float(learning_rate) / float(rms_value))

    if should_log_per_tensor_elr(update_step):
        relative_errors = [
            abs(actual - target) / max(abs(target), 1e-30)
            for actual, target in zip(actual_elrs, target_values)
        ]
        record = dict(
            update_step=update_step,
            controlled_tensor_count=len(controlled_names),
            elr_definition='learning_rate / parameter_rms',
            weight_decay=args.weight_decay,
            norm_projection=False,
            target_elr_min=min(target_values),
            target_elr_mean=sum(target_values) / len(target_values),
            target_elr_max=max(target_values),
            tensor_rms_min=min(rms_values),
            tensor_rms_mean=sum(rms_values) / len(rms_values),
            tensor_rms_max=max(rms_values),
        ...
        if (
            args.per_tensor_elr_detailed_log_every > 0
            and (
                update_step % args.per_tensor_elr_detailed_log_every == 0
                or update_step == 1
                or update_step == args.num_iterations
            )
        ):
            record['target_lr_over_tensor_norms'] = dict(zip(controlled_names, target_values))
            record['tensor_rms'] = dict(zip(controlled_names, rms_values))
            record['tensor_learning_rates'] = dict(zip(controlled_names, learning_rates))
        with open(os.path.join(logdir, 'per_tensor_rms_elr_history.jsonl'), 'a') as f:
            f.write(json.dumps(record) + '\n')

# Only RMSNorm gamma uses the original WSD schedule. The 73 controlled matrix
# LRs are overwritten directly from the external RMS-ELR file before each step.
...
gamma_scheduler = torch.optim.lr_scheduler.LambdaLR(gamma_optimizer, get_wsd_lr)
schedulers = [gamma_scheduler]
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    maybe_apply_per_tensor_rms_elr_controller(update_step)
