# Cosine-wave norm-control schedule collapse experiment.
# Delayed capture, RMS target ratio oscillates around 1.0, LR matched by the same ratio.
import os
import random
import sys
...
GLOBAL_NORM_CONTROL_KIND = 'global_mean_peak_lr_over_tensor_rms'
...
def norm_control_peak_lr(entry):
    return args.embed_learning_rate if canonical_param_name(entry['name']) == 'transformer.wte.weight' else 0.5 * args.embed_learning_rate

def global_control_metric(norm_control_state):
    if GLOBAL_NORM_CONTROL_KIND == 'global_total_fro_norm':
    ...
    values = []
    for entry in norm_control_state['params']:
        rms = entry['param'].detach().float().square().mean().sqrt()
        if rms <= norm_control_state['eps']:
            raise RuntimeError(f"near-zero RMS for global mean lr/norm control: {entry['name']}")
        values.append(norm_control_peak_lr(entry) / rms)
    return torch.stack(values).mean()

def tensor_rms_snapshot(norm_control_state):
    return {
        entry['name']: entry['param'].detach().float().square().mean().sqrt().item()
        for entry in norm_control_state['params']
    }

@torch.no_grad()
def apply_global_norm_control(norm_control_state, step, event):
    if not norm_control_state['enabled']:
        return
    mode = norm_control_state['mode']
    if mode != 'delayed_captured_constant':
        raise ValueError('global norm-control scripts require delayed_captured_constant mode')
    ...
    metric_before = global_control_metric(norm_control_state)
    rms_before = tensor_rms_snapshot(norm_control_state) if should_log else None
    captured = phase == 'capture'
    projected = phase == 'post_start'
    if captured:
        norm_control_state['reference_value'] = metric_before.item()
    reference = norm_control_state.get('reference_value')
    target = None
    scale = torch.ones((), dtype=torch.float32, device=metric_before.device)
    if projected:
        if reference is None:
            raise RuntimeError('missing captured reference for global norm control')
        target = reference * ratio if GLOBAL_NORM_CONTROL_KIND == 'global_total_fro_norm' else reference / ratio
        if metric_before <= norm_control_state['eps']:
            raise RuntimeError('near-zero aggregate metric for global norm control')
        if GLOBAL_NORM_CONTROL_KIND == 'global_total_fro_norm':
            scale = torch.as_tensor(target, dtype=torch.float32, device=metric_before.device) / metric_before
        else:
            scale = metric_before / torch.as_tensor(target, dtype=torch.float32, device=metric_before.device)
        for entry in norm_control_state['params']:
            entry['param'].mul_(scale.to(dtype=entry['param'].dtype, device=entry['param'].device))
    elif captured:
        target = reference
    metric_after = global_control_metric(norm_control_state)
    if should_log:
        relative_error = None if target is None else abs(metric_after.item() - target) / target
        wsd_ratio = get_wsd_lr(step)
        if GLOBAL_NORM_CONTROL_KIND == 'global_total_fro_norm':
            block_lr = 0.5 * args.embed_learning_rate * wsd_ratio * ratio
            aligned_value_before = block_lr / metric_before.item()
            aligned_value_after = block_lr / metric_after.item()
            aligned_reference_value = None if reference is None else 0.5 * args.embed_learning_rate * wsd_ratio / reference
        else:
            aligned_value_before = wsd_ratio * ratio * metric_before.item()
            aligned_value_after = wsd_ratio * ratio * metric_after.item()
            aligned_reference_value = None if reference is None else wsd_ratio * reference
        record = dict(
            step=step,
            event=event,
            mode=mode,
            controller_type=GLOBAL_NORM_CONTROL_KIND,
            phase=phase,
            schedule_ratio=ratio,
            reference_value=reference,
            target_value=target,
            pre_control_value=metric_before.item(),
            post_control_value=metric_after.item(),
            aligned_value_before=aligned_value_before,
            aligned_value_after=aligned_value_after,
            aligned_reference_value=aligned_reference_value,
            uniform_scale=scale.item(),
            uniform_divisor=(1.0 / scale.item()),
            relative_error=relative_error,
            projected=projected,
            captured=captured,
            start_step=start_step,
            controlled_tensor_count=len(norm_control_state['params']),
            pre_tensor_rms_by_name=rms_before,
            post_tensor_rms_by_name=tensor_rms_snapshot(norm_control_state),
            peak_lr_by_name={entry['name']: norm_control_peak_lr(entry) for entry in norm_control_state['params']},
            weight_decay=0.0,
        )
        with open(os.path.join(logdir, 'norm_control_history.jsonl'), 'a') as f:
...
def write_norm_control_targets(norm_control_state):
    ...
    records = dict(
        controller_type=GLOBAL_NORM_CONTROL_KIND,
        reference_value=norm_control_state.get('reference_value'),
        start_step=norm_control_state['start_step'],
        controlled_parameters=[entry['name'] for entry in norm_control_state['params']],
    )
...
def write_norm_control_metadata(norm_control_state):
    ...
    metadata = dict(
        ...
        mode=norm_control_state['mode'],
        controller_type=GLOBAL_NORM_CONTROL_KIND,
        norm_type=('global_total_fro' if GLOBAL_NORM_CONTROL_KIND == 'global_total_fro_norm' else 'arithmetic_mean_peak_lr_over_tensor_rms'),
        ...
        controlled_parameters=[
            dict(
                ...
                peak_lr=norm_control_peak_lr(entry),
...
apply_global_norm_control(norm_control_state, step=0, event='initial')
...
for step in range(args.num_iterations + 1):
    ...
    apply_global_norm_control(norm_control_state, step=update_step, event='post_step')
