# Signum cosine-wave norm-control schedule collapse experiment.
...
class Signum(torch.optim.Optimizer):
    """Signum: parameter -= lr * sign(EMA(gradient))."""

    def __init__(self, params, lr, momentum=0.95):
        if lr < 0.0:
            raise ValueError(f"invalid Signum learning rate: {lr}")
        if not 0.0 <= momentum < 1.0:
            raise ValueError(f"invalid Signum momentum: {momentum}")
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if g.is_complex():
                    raise RuntimeError("Signum does not support complex gradients")
                state = self.state[p]
                momentum_buffer = state.setdefault('momentum_buffer', torch.zeros_like(p))
                momentum_buffer.mul_(momentum).add_(g, alpha=1.0 - momentum)
                p.add_(momentum_buffer.sign(), alpha=-lr)
...
class Hyperparameters:
    ...
    signum_learning_rate : float = 0.00045 # Effective-update-matched Signum block baseline learning rate
    signum_momentum : float = 0.95
    ...
    signum_update_norm_every : int = 4 # every how many optimizer steps to log Signum effective update norms? 0 disables
...
non_matrix_block_parameters = [
    (name, tuple(p.shape)) for name, p in block_named_parameters if p.ndim != 2
]
if non_matrix_block_parameters:
    raise RuntimeError(f"SignSGD comparison expects the Muon 2D block-parameter set: {non_matrix_block_parameters}")
...
if args.weight_decay != 0:
    raise RuntimeError("this Signum norm-control experiment requires weight_decay=0")
optimizer2_groups = []
if uncontrolled_block_parameters:
    optimizer2_groups.append(dict(params=uncontrolled_block_parameters))
if controlled_block_parameters:
    optimizer2_groups.append(dict(params=controlled_block_parameters))
optimizer2 = Signum(optimizer2_groups, lr=args.signum_learning_rate, momentum=args.signum_momentum)
optimizers = [optimizer1, optimizer2]
if rmsnorm_gamma_parameters:
    optimizer3 = torch.optim.AdamW(rmsnorm_gamma_parameters, lr=0.5*args.embed_learning_rate,
...
schedulers = [
    ...
    torch.optim.lr_scheduler.LambdaLR(optimizer2, [get_lr for _ in optimizer2.param_groups]),
]
if rmsnorm_gamma_parameters:
    schedulers.append(torch.optim.lr_scheduler.LambdaLR(optimizer3, get_wsd_lr))
...
def should_log_signum_update_norms(update_step):
    if not master_process or args.signum_update_norm_every <= 0:
        return False
    return update_step % args.signum_update_norm_every == 0 or update_step == args.num_iterations

def maybe_capture_signum_update_state(update_step):
    if not should_log_signum_update_norms(update_step):
        return None
...
def signum_update_tensor(tensor, tensor_before, param_hparams, step, name):
    lr = param_hparams['lr']
    if lr == 0:
        raise RuntimeError(f"cannot infer Signum update for {name} at step {step}: lr is 0")
...
def signum_update_norm_record(step, name, tensor, signum_update, param_hparams, spectral_norm_estimate=None):
    lr = param_hparams['lr']
    weight_decay = param_hparams['weight_decay']
    record = dict(
        step=step,
    ...
    record.update(tensor_norm_fields(
        signum_update,
        prefix='signum_update_',
        spectral_norm_estimate=spectral_norm_estimate,
    ))
    return record

def maybe_log_signum_update_norms(update_step, update_state):
    if update_state is None:
        return
    history_path = os.path.join(logdir, 'signum_update_norm_history.jsonl')
    ...
    with torch.no_grad(), open(history_path, 'a') as f:
        for name, tensor in raw_model.named_parameters():
            ...
            signum_update = signum_update_tensor(tensor, tensor_before, hparams[param_id], update_step, name)
            if args.spectral_norm_estimate_enabled > 0 and signum_update.ndim == 2:
                pending_2d_updates.append((name, signum_update))
                pending_records.append((name, tensor, signum_update, hparams[param_id]))
            else:
                record = signum_update_norm_record(update_step, name, tensor, signum_update, hparams[param_id])
                f.write(json.dumps(record) + '\n')
                del record
            del tensor_before
        spectral_estimates = spectral_norm_estimates_by_name(pending_2d_updates)
        for name, tensor, signum_update, param_hparams in pending_records:
            record = signum_update_norm_record(
                update_step,
                name,
                tensor,
                signum_update,
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    signum_update_state = maybe_capture_signum_update_state(update_step)
    for opt in optimizers:
        opt.step()
    maybe_log_signum_update_norms(update_step, signum_update_state)
