...
# Architecture: noqknorm_layernorm; mode: matched; reference WD: 0.
...
GLOBAL_ELR_ARCHITECTURE = 'noqknorm_layernorm'
GLOBAL_ELR_MODE = 'matched'
GLOBAL_ELR_REFERENCE_WEIGHT_DECAY = 0.3
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mlp = MLP(config)
        self.attn_norm = LayerNorm(config.n_embd)
        self.mlp_norm = LayerNorm(config.n_embd)
...
class Hyperparameters:
    ...
    weight_decay : float = 0 # reference matrices+tied embedding; matched scripts use 0
    ...
    global_elr_reference_path : str = 'experiments/norm_control_schedule_collapse/reference_trajectories/noqknorm_layernorm_wd0p3_lr0p00045.jsonl'
...
block_named_parameters = [
    ...
    and not is_norm_gamma_name(name)
]
...
norm_gamma_parameters = [
    p
    for name, p in raw_model.named_parameters()
    if is_norm_gamma_name(name)
]
...
if norm_gamma_parameters:
    optimizer_groups.append(dict(params=norm_gamma_parameters, weight_decay=0.0))
