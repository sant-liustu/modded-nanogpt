# PlainSignSGD cosine-wave norm-control schedule collapse experiment.
...
# -----------------------------------------------------------------------------
# PlainSignSGD optimizer

class PlainSignSGD(torch.optim.Optimizer):
    """State-free plain SignSGD: parameter -= lr * sign(gradient)."""

    def __init__(self, params, lr):
        if lr < 0.0:
            raise ValueError(f"invalid SignSGD learning rate: {lr}")
...
class Hyperparameters:
    ...
    signsgd_learning_rate : float = 0.00045 # Effective-update-matched SignSGD block baseline learning rate
    ...
    signsgd_update_norm_every : int = 4 # every how many optimizer steps to log PlainSignSGD effective update norms? 0 disables
...
if args.weight_decay != 0:
    raise RuntimeError("this PlainSignSGD norm-control experiment requires weight_decay=0")
optimizer2_groups = []
...
optimizer2 = PlainSignSGD(optimizer2_groups, lr=args.signsgd_learning_rate)
...
def should_log_signsgd_update_norms(update_step):
    if not master_process or args.signsgd_update_norm_every <= 0:
        return False
    return update_step % args.signsgd_update_norm_every == 0 or update_step == args.num_iterations

def maybe_capture_signsgd_update_state(update_step):
    if not should_log_signsgd_update_norms(update_step):
        return None
...
def signsgd_update_tensor(tensor, tensor_before, param_hparams, step, name):
    lr = param_hparams['lr']
    if lr == 0:
        raise RuntimeError(f"cannot infer PlainSignSGD update for {name} at step {step}: lr is 0")
...
def signsgd_update_norm_record(step, name, tensor, signsgd_update, param_hparams, spectral_norm_estimate=None):
    lr = param_hparams['lr']
    weight_decay = param_hparams['weight_decay']
    record = dict(
        step=step,
    ...
    record.update(tensor_norm_fields(
        signsgd_update,
        prefix='signsgd_update_',
        spectral_norm_estimate=spectral_norm_estimate,
    ))
    return record

def maybe_log_signsgd_update_norms(update_step, update_state):
    if update_state is None:
        return
    history_path = os.path.join(logdir, 'signsgd_update_norm_history.jsonl')
    ...
    with torch.no_grad(), open(history_path, 'a') as f:
        for name, tensor in raw_model.named_parameters():
            ...
            signsgd_update = signsgd_update_tensor(tensor, tensor_before, hparams[param_id], update_step, name)
            if args.spectral_norm_estimate_enabled > 0 and signsgd_update.ndim == 2:
                pending_2d_updates.append((name, signsgd_update))
                pending_records.append((name, tensor, signsgd_update, hparams[param_id]))
            else:
                record = signsgd_update_norm_record(update_step, name, tensor, signsgd_update, hparams[param_id])
                f.write(json.dumps(record) + '\n')
                del record
            del tensor_before
        spectral_estimates = spectral_norm_estimates_by_name(pending_2d_updates)
        for name, tensor, signsgd_update, param_hparams in pending_records:
            record = signsgd_update_norm_record(
                update_step,
                name,
                tensor,
                signsgd_update,
...
for step in range(args.num_iterations + 1):
    ...
    update_step = step + 1
    signsgd_update_state = maybe_capture_signsgd_update_state(update_step)
    for opt in optimizers:
        opt.step()
    maybe_log_signsgd_update_norms(update_step, signsgd_update_state)
