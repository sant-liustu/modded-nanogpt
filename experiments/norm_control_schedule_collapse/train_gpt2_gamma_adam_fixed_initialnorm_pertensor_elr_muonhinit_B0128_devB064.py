# AdamW fixed-initial-norm arm for per-tensor ELR-govern experiments.
# MuonH-compatible block initialization; RMS ELR targets drive per-tensor LR.
import os
import random
import sys
...
class Hyperparameters:
    ...
    seed : int = 0
    norm_control_config : str = 'experiments/norm_control_schedule_collapse/fixed_initial_norm_all_matrices_start0.json'
    per_tensor_elr_file : str = '' # required RMS-ELR JSONL/JSONL.GZ; canonical field: target_lr_over_tensor_rms
...
if len(expected_elr_names) != 73 or 'transformer.wte.weight' not in expected_elr_names:
    raise RuntimeError(
        "fixed-initial-norm ELR control requires exactly 72 block matrices plus "
...
def write_per_tensor_elr_metadata():
    ...
    metadata = dict(
        ...
        optimizer='AdamW',
        weight_decay=0.0,
        projection='post_step_to_step0_rms',
    )
