...
# Q/K RMSNorm has no learnable gamma; block pre-norm and final-norm gamma remain learnable.
import os
import random
import sys
...
class RMSNorm(nn.Module):
    ...
    def __init__(self, dim, eps=None, learnable=True):
        super().__init__()
        if learnable:
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.q_norm = RMSNorm(self.head_dim, learnable=False)
        self.k_norm = RMSNorm(self.head_dim, learnable=False)
...
qk_gamma_parameters = [
    ...
    if canonical_param_name(name).endswith(('.q_norm.weight', '.k_norm.weight'))
]
if qk_gamma_parameters:
    raise RuntimeError(f"Q/K RMSNorm gamma must be non-learnable: {qk_gamma_parameters}")
