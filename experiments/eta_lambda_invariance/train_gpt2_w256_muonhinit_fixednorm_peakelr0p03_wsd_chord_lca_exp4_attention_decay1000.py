...
def tensor_wsd_ratio(name, it):
    """Single intervention: exp4_attention_decay1000; all non-target tensors use base WSD."""
    targeted = name.startswith('transformer.h.') and any(name.endswith(suffix) for suffix in ('.attn.c_q.weight', '.attn.c_k.weight', '.attn.c_v.weight', '.attn.c_proj.weight'))
    # it=1000 is the state after 1000 updates, before update 1001.
    if targeted and it >= 1000:
        return (args.num_iterations - it) / (args.num_iterations - 1000)
    return get_lr(it)
...
def apply_tensor_rms_elr(optimizers, update_step, history_path=None):
    ...
    for opt in optimizers:
        for group in opt.param_groups:
            ...
            p = group['params'][0]
            name = tensor_name_by_id[id(p)]
            target = args.peak_rms_elr * tensor_wsd_ratio(name, update_step - 1)
...
# begin logging
if master_process:
    run_id = f'w256_muonhinit_fixed_initial_rms_adamw_peakelr{args.peak_rms_elr:g}_wsd_wd0_exp4_attention_decay1000_chord_simpson3every{args.simpson3_every}_s{args.seed}_' + str(uuid.uuid4())
...
if master_process:
    with open(os.path.join(logdir, 'norm_control_metadata.json'), 'w') as f:
        json.dump(dict(mode='fixed_initial_rms', start_step=0, weight_decay=args.weight_decay,
                       ...
                       intervention='exp4_attention_decay1000', gamma_schedule='base WSD',
                       elr_schedule='WSD with named single-module intervention', warmup_iters=args.warmup_iters, warmdown_iters=args.warmdown_iters,
