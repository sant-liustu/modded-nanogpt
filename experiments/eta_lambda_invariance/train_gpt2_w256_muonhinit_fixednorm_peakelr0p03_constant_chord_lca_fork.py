...
parser = argparse.ArgumentParser(description='Constant prefix and complete-state chord LCA branches')
parser.add_argument('--branch', choices=['constant', 'embedding', 'mlp', 'attention', 'mlp_norm', 'all_norm'], default='constant')
...
# begin logging
if master_process:
    run_id = f'w256_muonhinit_fixed_initial_rms_adamw_peakelr{args.peak_rms_elr:g}_constantfork_{cli.branch}_wd0_simpson3every{args.simpson3_every}_s{args.seed}_' + str(uuid.uuid4())
...
def restore_training_state(state):
    ...
    if state['source_sha256'] != hashlib.sha256(code.encode()).hexdigest():
        raise RuntimeError('checkpoint source differs; use the exact same script')
...
if cli.resume:
    ...
    if master_process:
        (Path(logdir) / 'resume_metadata.json').write_text(json.dumps(dict(
            ...
            branch=cli.branch, cumulative_lca_includes_prefix=True), indent=2))
