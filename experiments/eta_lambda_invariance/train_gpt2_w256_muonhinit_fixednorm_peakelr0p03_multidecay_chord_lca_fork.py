...
# Compatibility is restricted to the verified published constant-fork runner.
# Its sha256 uses Python universal-newline text, exactly as the old checkpoint did.
LEGACY_CONSTANT_SOURCE_SHA256 = "a7536f90740381c5859caf8d35d7f4dfbd86f35f09de359c0c7cb9b912dc15f7"
parser = argparse.ArgumentParser(description='Resume a constant checkpoint with any subset of matrix groups decaying')
parser.add_argument('--resume', type=Path, required=True, help='complete constant checkpoint directory')
parser.add_argument('--decay-groups', nargs='+', choices=['embedding', 'mlp', 'attention'], default=[],
                    help='omit for all-constant; Norm gamma always stays constant')
parser.add_argument('--fork-step', type=int, default=3650)
cli = parser.parse_args()
if len(cli.decay_groups) != len(set(cli.decay_groups)):
    parser.error('each decay group must appear only once')
cli.decay_groups = tuple(g for g in ('embedding', 'mlp', 'attention') if g in cli.decay_groups)
cli.branch = '+'.join(cli.decay_groups) or 'constant'
...
if not (cli.resume / 'complete.json').is_file():
...
def tensor_schedule_ratio(name, it):
    # Disjoint matrix groups only; no gamma can be selected by these suffixes.
    selected = (
        ('embedding' in cli.decay_groups and name == 'transformer.wte.weight')
        or ('mlp' in cli.decay_groups and name.endswith(('.mlp.c_fc.weight', '.mlp.c_proj.weight')))
        or ('attention' in cli.decay_groups and name.endswith((
            '.attn.c_q.weight', '.attn.c_k.weight', '.attn.c_v.weight', '.attn.c_proj.weight')))
    )
...
# begin logging
if master_process:
    run_id = f'w256_muonhinit_fixed_initial_rms_adamw_peakelr{args.peak_rms_elr:g}_multidecay_{cli.branch}_wd0_simpson3every{args.simpson3_every}_s{args.seed}_' + str(uuid.uuid4())
...
if master_process:
    with open(os.path.join(logdir, 'norm_control_metadata.json'), 'w') as f:
        json.dump(dict(mode='fixed_initial_rms', start_step=0, weight_decay=args.weight_decay,
                       ...
                       elr_schedule='warmup_constant_selected_decay', branch=cli.branch,
                       decay_groups=cli.decay_groups, norm_gamma_schedule='constant',
...
def restore_training_state(state):
    ...
    if state['source_sha256'] != LEGACY_CONSTANT_SOURCE_SHA256:
        raise RuntimeError(
            'unsupported checkpoint source: ' + str(state['source_sha256']) +
            '; expected the published constant-fork runner from commit df71952. '
            'Do not bypass this check; audit any modified source first.')
...
if cli.resume:
    ...
    if manifest.get('version') != 1 or manifest.get('step') != cli.fork_step:
        raise RuntimeError('checkpoint manifest version or step mismatch')
    if manifest.get('files') != [f'rank{r:05d}.pt' for r in range(ddp_world_size)]:
        raise RuntimeError('checkpoint manifest rank files mismatch')
    ...
    if master_process:
        (Path(logdir) / 'resume_metadata.json').write_text(json.dumps(dict(
            ...
            branch=cli.branch, decay_groups=cli.decay_groups,
            checkpoint_source_sha256=checkpoint_source_sha256,
            runner_source_sha256=hashlib.sha256(code.encode()).hexdigest(),
            norm_gamma_schedule='constant', cumulative_lca_includes_prefix=True), indent=2))
