...
parser = argparse.ArgumentParser(description='Resume constant checkpoint: MLP decay plus embedding ELR ramp')
parser.add_argument('--branch', choices=['mlp'], default='mlp')
parser.add_argument('--resume', type=Path, required=True)
parser.add_argument('--embedding-final-multiplier', type=float, choices=[1.0, 1.1, 1.2, 1.3, 1.4], default=1.0)
parser.add_argument('--checkpoint-dir', type=Path, default=Path('checkpoints/constant_step3650'))
parser.add_argument('--fork-step', type=int, default=3650)
parser.add_argument('--stop-at-fork', action='store_true')
cli = parser.parse_args()
if not args.warmup_iters <= cli.fork_step < args.num_iterations:
    parser.error('fork step must be after warmup and before training ends')
if args.simpson3_every <= 0 or cli.fork_step % args.simpson3_every:
    parser.error('fork step must be a Simpson interval boundary')
if cli.branch != 'constant' and cli.resume is None:
    parser.error('interventions must resume the common constant checkpoint')
if cli.resume and cli.stop_at_fork:
    parser.error('--stop-at-fork is only for creating the common prefix')
if not cli.resume and cli.checkpoint_dir.exists():
    parser.error('checkpoint directory already exists; choose a new directory or --resume')
if cli.resume and not (cli.resume / 'complete.json').is_file():
    parser.error('checkpoint is incomplete: complete.json is missing')
...
def tensor_schedule_ratio(name, it):
    # First resumed update starts at 1x; final actual update reaches the requested multiplier.
    if name == 'transformer.wte.weight' and it >= cli.fork_step:
        ramp_updates = args.num_iterations - cli.fork_step
        if ramp_updates < 2:
            raise RuntimeError('embedding ramp requires at least two updates')
        progress = min(1.0, max(0.0, (it-cli.fork_step)/(ramp_updates-1)))
        return 1.0 + (cli.embedding_final_multiplier-1.0)*progress
    selected = (
        (cli.branch == 'embedding' and name == 'transformer.wte.weight')
        or (cli.branch == 'mlp' and (name.endswith('.mlp.c_fc.weight') or name.endswith('.mlp.c_proj.weight')))
        or (cli.branch == 'attention' and '.attn.' in name and not is_rmsnorm_gamma_name(name))
        or (cli.branch == 'mlp_norm' and name.endswith('.mlp_norm.weight'))
        or (cli.branch == 'all_norm' and is_rmsnorm_gamma_name(name))
    )
    if selected and it >= cli.fork_step:
        return max(0.0, (args.num_iterations-it)/(args.num_iterations-cli.fork_step))
    return get_lr(it)
...
def apply_tensor_rms_elr(optimizers, update_step, history_path=None):
    ...
    for opt in optimizers:
        for group in opt.param_groups:
            ...
            target = args.peak_rms_elr * tensor_schedule_ratio(tensor_name_by_id[id(p)], update_step-1)
            group['lr'] = target * rms
...
# begin logging
if master_process:
    run_id = f'w256_muonhinit_fixed_initial_rms_adamw_peakelr{args.peak_rms_elr:g}_mlpdecay_embincrease{cli.embedding_final_multiplier:g}_wd0_simpson3every{args.simpson3_every}_s{args.seed}_' + str(uuid.uuid4())
...
if master_process:
    with open(os.path.join(logdir, 'norm_control_metadata.json'), 'w') as f:
        json.dump(dict(mode='fixed_initial_rms', start_step=0, weight_decay=args.weight_decay,
                       ...
                       elr_schedule='mlp_decay_embedding_linear_increase', branch=cli.branch,
                       embedding_final_multiplier=cli.embedding_final_multiplier,
                       fork_step=cli.fork_step, warmup_iters=args.warmup_iters,
                       warmdown_iters=args.num_iterations-cli.fork_step,
...
def capture_training_state(step, x, y):
    ...
    return checkpoint_tree(dict(
        version=1, step=step, branch=cli.branch, fork_step=cli.fork_step,
...
def restore_training_state(state):
    ...
    # Audited parent: df71952, unchanged full-state constant fork runner.
    compatible_parent_sha256 = 'a7536f90740381c5859caf8d35d7f4dfbd86f35f09de359c0c7cb9b912dc15f7'
    if state['source_sha256'] != compatible_parent_sha256:
        raise RuntimeError('checkpoint must come from the audited constant fork runner df71952; do not bypass this check for modified cloud scripts')
    ...
    if state['branch'] != 'constant' or state['step'] != cli.fork_step or state['fork_step'] != cli.fork_step:
        raise RuntimeError('expected the common constant checkpoint at the fork step')
...
if cli.resume:
    ...
    payload_source_hash = payload['source_sha256']
    del payload
    if master_process:
        (Path(logdir) / 'resume_metadata.json').write_text(json.dumps(dict(
            checkpoint=str(cli.resume.resolve()), next_update=start_step+1,
            branch=cli.branch, embedding_final_multiplier=cli.embedding_final_multiplier,
            parent_source_sha256=payload_source_hash, cumulative_lca_includes_prefix=True), indent=2))
...
for step in range(start_step, args.num_iterations + 1):
    if not cli.resume and step == cli.fork_step:
