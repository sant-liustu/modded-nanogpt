...
def tensor_wsd_ratio(name, it):
    """Single intervention: exp3_attention_constant; all non-target tensors use base WSD."""
    ...
    if targeted and it >= args.warmup_iters:
        return 1.0
    return get_lr(it)
...
# begin logging
if master_process:
    run_id = f'w256_muonhinit_fixed_initial_rms_adamw_peakelr{args.peak_rms_elr:g}_wsd_wd0_exp3_attention_constant_chord_simpson3every{args.simpson3_every}_s{args.seed}_' + str(uuid.uuid4())
...
if master_process:
    with open(os.path.join(logdir, 'norm_control_metadata.json'), 'w') as f:
        json.dump(dict(mode='fixed_initial_rms', start_step=0, weight_decay=args.weight_decay,
                       ...
                       intervention='exp3_attention_constant', gamma_schedule='base WSD',
