...
class GPTConfig:
    ...
    n_layer : int = 12
    n_head : int = 3 # head dim 128 suggested by @Grad62304977
    n_embd : int = 384
    init_std : float = 0.02
    scale_emb : float = 1.0
...
class Hyperparameters:
    ...
    weight_decay : float = 0.05 # AdamW weight decay applied to tied wte/lm_head and transformer blocks
    seed : int = 0
...
num_vocab = 50304
model = GPT(GPTConfig(vocab_size=num_vocab, n_layer=12, n_head=3, n_embd=384))
...
# begin logging
if master_process:
    run_id = f'eta_lam_w384_lr0p0009_cos_wd0p05_lrn_w768lr0p0009_wd0p1_s{args.seed}_' + str(uuid.uuid4())
