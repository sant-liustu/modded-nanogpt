...
class Hyperparameters:
    ...
    spherical_norm_rtol : float = 1e-5 # reject genuine norm drift, retain rounding-level drift explicitly
...
# begin logging
if master_process:
    run_id = f'w256_muonhinit_fixed_initial_rms_adamw_peakelr{args.peak_rms_elr:g}_wsd_wd0_spherical_simpson3every{args.simpson3_every}_s{args.seed}_' + str(uuid.uuid4())
...
@torch.no_grad()
def spherical_geometry(start, end, norm_rtol=1e-5):
    """Small-angle-stable short-arc geometry, with exact checkpoint endpoints.

    Equal radii give a sphere. For rounding-level radius mismatch only, use
    linear radius times unit SLERP and include its radial derivative explicitly.
    No endpoint is silently projected. Substantial drift/antipodes are rejected.
    """
    left, right = start.detach().double(), end.detach().double()
    r0 = torch.linalg.vector_norm(left)
    r1 = torch.linalg.vector_norm(right)
    if not torch.isfinite(r0) or not torch.isfinite(r1) or r0 <= 0 or r1 <= 0:
        raise ValueError('spherical path requires finite nonzero endpoints')
    drift = ((r1-r0).abs() / torch.maximum(r0, r1)).item()
    if drift > norm_rtol:
        raise ValueError(f'spherical endpoint norm drift {drift:.3g} exceeds {norm_rtol}')
    u0, u1 = left / r0, right / r1
    half_angle = torch.atan2(torch.linalg.vector_norm(u1-u0), torch.linalg.vector_norm(u1+u0))
    cos_half = torch.cos(half_angle)
    if cos_half < 1e-6:
        raise ValueError('near-antipodal endpoints do not define a stable unique short arc')
    dtype = torch.float64 if start.dtype == torch.float64 else torch.float32
    return dict(r0=r0.to(dtype), r1=r1.to(dtype), half_angle=half_angle.to(dtype),
                cos_half=cos_half.to(dtype), sinc_half=torch.sinc(half_angle / torch.pi).to(dtype),
                angle=(2*half_angle).item(), relative_norm_drift=drift)


@torch.no_grad()
def spherical_state_velocity(start, end, geometry, s):
    """Return gamma(s) and d gamma(s)/ds, not a shared endpoint displacement.

    Centered SLERP uses sinc to avoid cancellation at tiny or zero angles.
    At equal radii its endpoint velocities are aD +/- bM; midpoint is cD.
    """
    r0, r1 = geometry['r0'], geometry['r1']
    left, right = start.to(r0.dtype), end.to(r0.dtype)
    u0, u1 = left/r0, right/r1
    midpoint, delta = (u0+u1)*0.5, u1-u0
    x, z = geometry['half_angle'], 2*s-1
    unit = (torch.cos(z*x)/geometry['cos_half'])*midpoint + (z*torch.sinc(z*x/torch.pi)/(2*geometry['sinc_half']))*delta
    tangent = (-2*x*torch.sin(z*x)/geometry['cos_half'])*midpoint + (torch.cos(z*x)/geometry['sinc_half'])*delta
    radius = (1-s)*r0 + s*r1
    state = left if s == 0 else right if s == 1 else radius*unit
    velocity = (r1-r0)*unit + radius*tangent
...
class LCADiagnostic:
    ...
    def __init__(self, model, raw_model, probe_batches, logdir, writer):
        ...
        self.spherical_names = {name for name, p in self.named_params if p.ndim == 2 and
                                (name.startswith('transformer.h.') or name == 'transformer.wte.weight')}
        self.path_geometry = {}
        ...
        metadata = dict(
            ...
            definition='spherical short-arc Simpson with node-specific velocities; uncontrolled parameters use chords',
            spherical_norm_rtol=args.spherical_norm_rtol,
            spherical_parameters=sorted(self.spherical_names),
            radius_roundoff_policy='linear radius times unit SLERP, including radial velocity; exact endpoints',
            integration_order='single-interval O(h^5) for smooth integrands; composite global O(h^4)',
    ...
    def _load_interpolation(self, end_state, alpha):
        for name, p in self.named_params:
            ...
            if alpha == 0.0:
                ...
                if name in self.path_geometry:
                    state, _ = spherical_state_velocity(start, end, self.path_geometry[name], alpha)
                    p.copy_(state)
                else:
    ...
    @torch.no_grad()
    def _local_attributions(self, gradients, end_state, alpha):
        values = []
        for name, _ in self.named_params:
            if name in self.path_geometry:
                _, velocity = spherical_state_velocity(self.snapshot[name], end_state[name], self.path_geometry[name], alpha)
            else:
                dtype = torch.float64 if end_state[name].dtype == torch.float64 else torch.float32
                velocity = end_state[name].to(dtype) - self.snapshot[name].to(dtype)
            values.append(torch.sum(gradients[name].to(velocity.dtype)*velocity))
    ...
    def _evaluate(self, end_state, alpha, cache_endpoint=False):
        ...
        attributions = self._local_attributions(gradients, end_state, alpha)
    ...
    def _write_records(self, method, end_step, loss_start, loss_end, attributions, end_state):
        ...
        common = dict(method=method, step_start=self.snapshot_step, step_end=end_step,
                      integration_path='sphere_for_controlled_matrices_chord_for_vectors',
                      max_spherical_angle=max((g['angle'] for g in self.path_geometry.values()), default=0.0),
                      max_spherical_norm_drift=max((g['relative_norm_drift'] for g in self.path_geometry.values()), default=0.0),
    ...
    def run(self, method, end_step):
        ...
        # Only scalars are retained; full node states/velocities are formed one
        # tensor at a time. Invalid geometry fails before touching live weights.
        self.path_geometry = {name: spherical_geometry(self.snapshot[name], end_state[name], args.spherical_norm_rtol)
                              for name, _ in self.named_params if name in self.spherical_names}
        ...
        try:
            if self.endpoint_cache is not None and self.endpoint_cache['step'] == self.snapshot_step:
                ...
                grad_start = self._local_attributions(self.endpoint_cache['gradients'], end_state, 0.0)
            else:
