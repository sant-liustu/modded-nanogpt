...
# begin logging
if master_process:
    run_id = f'w256_muonhinit_fixed_initial_rms_adamw_jsonelr_{schedule_sha256[:12]}_wd0_simpson3every{args.simpson3_every}_s{args.seed}_' + str(uuid.uuid4())
...
def restore_training_state(state):
    ...
    if state['source_sha256'] not in {LEGACY_CONSTANT_SOURCE_SHA256, hashlib.sha256(code.encode()).hexdigest()}:
        raise RuntimeError('unsupported checkpoint source; expected audited df71952 parent or this exact JSON runner')
