...
class Hyperparameters:
    ...
    parent_run_id : str = ''
    # one-shot intervention hyperparams
    intervention_type : str = 'none' # none | rmsnorm_gamma_lognormal
    intervention_target : str = 'rmsnorm_gamma'
    intervention_scale : float = 0.0 # log-space std for rmsnorm_gamma_lognormal
    intervention_seed : int = 1337
...
def parse_args():
    ...
    assert args.intervention_type in ("none", "rmsnorm_gamma_lognormal"), \
        "intervention_type must be none or rmsnorm_gamma_lognormal"
    assert args.intervention_target == "rmsnorm_gamma", "only rmsnorm_gamma intervention_target is supported"
    assert args.intervention_scale >= 0, "intervention_scale must be non-negative"
    assert args.intervention_type == "none" or args.resume_mode == "fork", \
        "interventions are only supported for fork resume"
...
def validate_rmsnorm_gamma_checkpoint(checkpoint):
    if checkpoint is None:
        return
    has_gamma = any(key.endswith("norm.weight") for key in checkpoint.get("model", {}))
    assert has_gamma, "checkpoint does not contain RMSNorm gamma parameters; use this script to create the parent checkpoint first"
...
validate_rmsnorm_gamma_checkpoint(checkpoint)
...
def optimizer_lrs():
    return dict(
        ...
        lr_rmsnorm=optimizers[2].param_groups[0]["lr"],
    )
...
def get_diagnostic_parameter_info(name, parameter):
    ...
    parts = name.split(".")
    if len(parts) == 6 and parts[0] == "transformer" and parts[1] == "h" and parts[-1] == "weight":
        component = f"{parts[3]}.{parts[4]}"
        if parameter.ndim == 2 and component in diagnostic_target_components:
            return dict(name=name, layer=int(parts[2]), component=component, parameter_kind="matrix")
        if parameter.ndim == 1 and component in rmsnorm_gamma_components:
            return dict(name=name, layer=int(parts[2]), component=component, parameter_kind="rmsnorm_gamma")

    if len(parts) == 5 and parts[0] == "transformer" and parts[1] == "h" and parts[-1] == "weight":
        component = parts[3]
        if parameter.ndim == 1 and component in rmsnorm_gamma_components:
            return dict(name=name, layer=int(parts[2]), component=component, parameter_kind="rmsnorm_gamma")

    if len(parts) == 2 and parts[0] == "norm" and parts[1] == "weight" and parameter.ndim == 1:
        return dict(name=name, layer=None, component="final_norm", parameter_kind="rmsnorm_gamma")
...
def maybe_log_diagnostics(global_step, force=False):
    ...
    for info, parameter in iter_diagnostic_parameters():
        ...
        records.append(dict(
            ...
            parameter=info["name"],
            parameter_kind=info["parameter_kind"],
            layer=info["layer"],
...
def iter_rmsnorm_gamma_parameters():
    for info, parameter in iter_diagnostic_parameters():
        if info["parameter_kind"] == "rmsnorm_gamma":
            yield info, parameter


@torch.no_grad()
def apply_intervention(global_step):
    metadata = dict(
        type=args.intervention_type,
        target=args.intervention_target,
    ...
    if args.intervention_type == "none" or args.resume_mode != "fork":
        return metadata

    generator = torch.Generator(device=device)
    generator.manual_seed(args.intervention_seed)
    for info, parameter in iter_rmsnorm_gamma_parameters():
        before = parameter.detach().float().clone()
        noise = torch.randn(parameter.shape, generator=generator, device=parameter.device, dtype=torch.float32)
        factor = torch.exp(args.intervention_scale * noise)
        parameter.mul_(factor.to(parameter.dtype))
        after = parameter.detach().float()
        before_norm = before.norm()
        after_norm = after.norm()
        relative_delta = None
        if before_norm.item() > 0:
            relative_delta = float(((after - before).norm() / before_norm).item())
        metadata["parameters"].append(dict(
            parameter=info["name"],
            layer=info["layer"],
            component=info["component"],
            before_norm=float(before_norm.item()),
            after_norm=float(after_norm.item()),
            norm_ratio=float((after_norm / before_norm).item()) if before_norm.item() > 0 else None,
            relative_delta=relative_delta,
            factor_mean=float(factor.mean().item()),
            factor_std=float(factor.std(unbiased=False).item()),
    ...
    if master_process:
        with open(os.path.join(logdir, "intervention.json"), "w") as f:
            json.dump(metadata, f, indent=2, sort_keys=True)
        with open(logfile, "a") as f:
            f.write(f"intervention: {json.dumps(metadata, sort_keys=True)}\n")
    return metadata

# init the optimizer(s)
hidden_matrix_params = []
rmsnorm_gamma_params = []
for name, parameter in raw_model.named_parameters():
    clean_name = clean_parameter_name(name)
    info = get_diagnostic_parameter_info(clean_name, parameter)
    if info is not None and info["parameter_kind"] == "rmsnorm_gamma":
        rmsnorm_gamma_params.append(parameter)
    elif clean_name.startswith("transformer.h."):
...
optimizer2 = torch.optim.AdamW(hidden_matrix_params, lr=0.5*args.embed_learning_rate, betas=(0.9, 0.95),
                               weight_decay=args.weight_decay, fused=True)
optimizer3 = torch.optim.AdamW(rmsnorm_gamma_params, lr=0.5*args.embed_learning_rate, betas=(0.9, 0.95),
...
if checkpoint is not None:
    ...
    assert len(checkpoint["optimizers"]) == len(optimizers), "optimizer count changed between checkpoint and resume"
    assert len(checkpoint["schedulers"]) == len(schedulers), "scheduler count changed between checkpoint and resume"
...
if checkpoint is not None and args.resume_mode == "exact" and "intervention_metadata" in checkpoint:
    intervention_metadata = checkpoint["intervention_metadata"]
else:
    intervention_metadata = apply_intervention(global_step)
...
while global_step < args.num_iterations:
    ...
    for i in range(1, train_accumulation_steps+1):
        ...
        if i < train_accumulation_steps:
            ...
            loss.backward()
    train_loss = train_loss_accum / train_accumulation_steps
